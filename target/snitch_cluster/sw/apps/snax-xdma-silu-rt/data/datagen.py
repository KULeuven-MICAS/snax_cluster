...
# matching silu goldens. The SAME xDMA netlist (StreamMapRt with func=SILU) runs
...
# at the stage boundary): the grid-snapped input is fed to one StreamMapRt SILU
# pass -- silu(x) = x * sigmoid(x) = x / (1 + exp(-x)), computed in f64 -- and the
# result is narrowed back to the transport format. silu is signed (small negative
...
def silu_chain_fp8(xf):
    ...
    silu_f32 = (xd * sig).astype(np.float32)
    y8 = f32_to_e5m2_bits(silu_f32)                       # map SILU output (fmt-narrowed)
    return x8, y8
...
def emit_header_file(**kwargs):
    ...
    x16u, g16u = silu_chain_fp16(xf)
    x8u, g8u = silu_chain_fp8(xf)

    emit = ["#include <stdint.h>"]
    emit += [format_scalar_definition("uint32_t", "silu_n", n)]
    emit += [format_scalar_definition("uint32_t", "silu_beats_fp16", n // FP16_PER_BEAT)]
    emit += [format_scalar_definition("uint32_t", "silu_beats_fp8", n // FP8_PER_BEAT)]
    # FP16 dataset
    emit += [format_vector_definition("uint16_t", "silu_input_fp16", x16u,
                                      alignment=64, hex_bits=16, cast_hex=True)]
    emit += [format_vector_definition("uint16_t", "silu_golden_fp16", g16u,
                                      alignment=64, hex_bits=16, cast_hex=True)]
    # FP8 dataset
    emit += [format_vector_definition("uint8_t", "silu_input_fp8", x8u,
                                      alignment=64, hex_bits=8, cast_hex=True)]
    emit += [format_vector_definition("uint8_t", "silu_golden_fp8", g8u,
                                      alignment=64, hex_bits=8, cast_hex=True)]
    return "\n\n".join(emit)


def main():
    parser = argparse.ArgumentParser(description="Data for the runtime-precision xDMA silu kernel")
