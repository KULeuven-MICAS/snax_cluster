...
//
// SIMD FP16 multi-row RMSNorm, A/B: per row, out[r,:] = x[r,:] * inv_rms[r],
// with inv_rms[r] = 1/sqrt(mean(x[r,:]^2)). Same kernel, tensor and golden as
// snax-simd-rmsnorm-multirow; this app runs the DEPLOYED path and a
// SOFTWARE-ONLY optimized path back to back on the same data, and reports both.
//
// The per-row scalar cannot be held constant across a row by the reader AGU (a
// single affine address stream cannot keep one operand's address fixed while
// the other walks the row), so inv_rms[r] must be materialized as a full
// [rows,D] operand tensor -- unavoidable without an RTL change. What IS
// avoidable is materializing it with the DM core, and re-reading the result for
// a separate quantize pass:
//
//   A (deployed)
//     T1  StreamReduce(SUMSQ)              x -> ssq[rows]
...
//   inv_bc_b == inv_bc_a  the stride-0 broadcast replicates the beat exactly
...
// deployed app does.
//
// inv_rms[] is DMA'd to L1 up front and BOTH paths read it from there: in
// data.h it lives in L3, and a per-row DRAM load (~146 cc/row) would tax both
// core loops for reasons that have nothing to do with the kernel. Zeroing the
// lane-0 beat buffer is likewise a one-time scratch init (lanes 1..31 stay zero
// across calls), so it is not timed.
//
// MEASURED (vsim, warm, L1<->L1). A's xDMA task cycles match
// snax-simd-rmsnorm-multirow exactly at 4x64 (sumsq 197, scale 144), so A is
// the deployed kernel, not a strawman. The win scales with D: A's broadcast
// loop is rows*D core stores (~4 cc each), B's splat is rows*16, independent of
// D.
//
...
// L1 map (byte offset from cluster base; rows_bytes = rows*beats*64 = 8 KiB):
...
//   +9280   inv_lane0 1024 B  [16] inv_rms[r] splatted, one beat per row (B0)
//   +10304  inv_bc_a  8192 B  [16,256] bcast, DM-core loop  (A1; A2 operand 1)
//   +18496  inv_bc_b  8192 B  [16,256] bcast, xDMA stride-0 (B1; B2 operand 1)
...
int main() {
    ...
    if (snax_is_simd_core()) {
        ...
        uint8_t* inv_bc_b =
            inv_bc_a + rows_bytes;  // [rows,D] bcast, xDMA stride-0
        ...
        uint8_t* q_fused =
            q_sep + rows_bytes / 2;  // int8, quant chained onto the EW

        printf("[RmsFold] rows=%u D=%u beats=%u\n", rows, d, beats);

        // Input + the per-row inv_rms scalars into L1 (both core loops read
        // inv_rms from L1, so neither pays a per-row DRAM load for a data.h
        // array).
        snax_stage_1d(x_in, rmsf_input, rows_bytes);
        snax_stage_1d(inv_l1, rmsf_inv_rms, rows * sizeof(uint16_t));

        // One-time scratch init: lanes 1..31 of each lane-0 beat stay zero
        // across calls, so B0 only ever rewrites lane 0. Not part of the
        // per-call cost.
        for (uint32_t i = 0; i < scal_bytes / 4; i++)
            ((uint32_t*)inv_lane0)[i] = 0;
        ...
        // Per-stage wall-clock (CSR config + start + wait), and the xDMA
        // datapath-only cycles. Two iterations: 0 = cold (one-time AGU shape
        // setup + icache), 1 = warm.
        ...
        for (int it = 0; it < 2; it++) {
            ...
            // B0: splat inv_rms[r] across the 32 lanes of ONE beat per row, as
            // 16 word stores (2 FP16 lanes each). rows*16 stores -- O(rows),
            // independent of D (the A1 loop is rows*D). Doing this splat in HW
            // instead (a stride-0 read through StreamReduce(MAX,
            // operandCount=1)) is NOT possible on this RTL: a 1-beat row
            // retires every cycle and trips StreamReduce's "row completed while
            // the fold was still busy" assert -- the pipelined horizontal fold
            ...
            // B1: xDMA broadcast -- reader inner stride 0 re-reads the row's
            ...
            // B2: out_b = x (.) inv_bc_b, with Fp16ToInt8 CHAINED -- one task
            // emits both the FP16 scale result (through the chain) and the int8
            // quant. The writer drains the int8 stream (all_beats/2); out_b is
            ...
            // B2': the same EW MUL WITHOUT the chained quant -- path B's
        ...
        printf(
            "[RmsFold] --- stage cycles (warm; wall = CSR cfg + start + wait) "
            "---\n");
        ...
        for (uint32_t i = 0; i < rows * d; i++) {
            ...
            if (ulp > 4) {
                if (mism < 6)
                    printf(
                        "[RmsFold] mismatch[%u] (row %u): got %04x golden "
                        "%04x\n",
                        i, i / d, oa[i], rmsf_out_golden[i]);
                mism++;
            }
        }
        ...
        // 2) the stride-0 broadcast must reproduce the DM-core fill
        // BIT-EXACTLY.
        uint32_t bad = 0;
        ...
        // 4) chaining the quant onto the EW pass must be bit-identical to a
        // separate pass.
