...
//
// xDMA FP16 RoPE (Llama3, interleaved adjacent-pair / complex-rotation
// convention). Pair k = (x[2k], x[2k+1]) rotates by angle theta_k:
//   out[2k]   = x[2k]*cos_k - x[2k+1]*sin_k
//   out[2k+1] = x[2k]*sin_k + x[2k+1]*cos_k
//
// ZERO new RTL: reuse the existing StreamElementwise extension (MUL/ADD). RoPE
// is 3 passes over per-pair-duplicated tables, plus one adjacent halfword swap
// of x:
//   swap  xswap = [x1,x0,x3,x2,...]                       (host, integer:
//   16-bit rotate per word) P1    tmp1  = x     (.) cos_full StreamElementwise
//   MUL   cos_full   = [c0,c0,c1,c1,...] P2    tmp2  = xswap (.) sin_signed
//   StreamElementwise MUL   sin_signed = [-s0,+s0,-s1,+s1,...] P3    out   =
//   tmp1  (+) tmp2       StreamElementwise ADD
// The rotate_half sign lives in the precomputed sin_signed table (free); the
// only runtime data move is the adjacent swap, cheap because pairs are
// adjacent. FP16 transport, FP32-internal; out matches the FP64 golden to <=4
// FP16 ULP. Each pass reads its two operands as one INTERLEAVED stream (AGU
// inner dim count=2 striding operand0 -> operand1), exactly like swiglu's mul
// pass.
//
// LANE DATAFLOW (FP16 transport = 32 lanes/512-b beat; all 3 passes are
// StreamElementwise, 2 beats -> 1 beat, op per lane). RoPE works on adjacent
// PAIRS (lanes 2k, 2k+1 share angle theta_k), encoded purely in how the operand
// tables are laid out across the lanes -- no special HW:
//   lane:       0     1     2     3   ...          (pair k uses lanes 2k, 2k+1)
//   x        [  x0    x1    x2    x3  ... ]
//   xswap    [  x1    x0    x3    x2  ... ]         adjacent halfword swap
//   (iDMA, one-time) cos_full [  c0    c0    c1    c1  ... ]         each cos_k
//   duplicated across the pair sin_sgn  [ -s0   +s0   -s1   +s1  ... ]
//   rotate_half sign baked into the table P1 MUL:  tmp1 = x     (.) cos_full ->
//   [ x0*c0    x1*c0    x2*c1  ... ] P2 MUL:  tmp2 = xswap (.) sin_sgn  -> [
//   -x1*s0   x0*s0   -x3*s1  ... ] P3 ADD:  out  = tmp1  (+) tmp2     -> [
//   x0*c0-x1*s0   x1*c0+x0*s0  ... ]   (the rotation) Each pass is a clean
//   32-lane-in/32-lane-out element-wise op (the 2:1 is the interleaved-operand
//   read).
//
// Performance (FP16, vsim, L1<->L1), measured on the native-Chisel FP RTL
// (StreamElementwise mixed FMA, fpPipe=1 cut) + time-mux computeLanes=4. swap*
// = one-time rotate_half staging (iDMA, ~2 cyc/elem); p1/p2/p3 = the
// StreamElementwise passes; xdma_total = p1+p2+p3 (pure datapath); warm/cold =
// the 3-pass offload only (swap excluded, comparable to swiglu's warm/cold);
// per-call = swap+warm.
//
//   N      beats   swap*   p1     p2     p3     xdma_total   warm     cold
//   per-call(swap+warm)
//   ----   -----   -----   ----   ----   ----   ----------   ------   ------
//   ------------------- 64     2       166     48     48     48     144 1,740
//   1,912    1,906 256    8       551     144    144    144    432 2,028 2,200
//   2,579 1024   32      2,090   528    528    528    1,584        3,130 3,336
//   5,220 4096   128     8,235   2,064  2,064  2,064  6,192        7,738 7,944
//   15,973
//
// worst FP16 ULP vs the 3-pass golden = 0 (N<=1024), 1 (N=4096). The datapath
// is ~16 cyc/beat/pass (the computeLanes=4 time-mux; halved from cl=2); the
// ~1.6k fixed warm cost is the 3x CSR orchestration (3 memcpy_nd setups), not
// the math (skill: orchestration is the bottleneck). At large N the swap (the
// rotate_half tax of the zero-HW path) and the time-muxed datapath dominate.
// Reference: swiglu's measured host(full) silu+mul is ~33.6*N cyc
// (2,308..137,799 for N=64..4096); a host RoPE has no transcendental so it is
// cheaper, hence the offload pays off only at larger N. A fused single-pass
// StreamRoPE (intra-beat adjacent-pair) would remove the swap and 2 of the 3
// passes.
...
int main() {
    ...
    if (snrt_is_dm_core()) {
        ...
        uint32_t sw = 0;
        {
            ...
            snrt_dma_start_2d(xswap, x_in + 2, 2, 4, 4,
                              pairs);  // xswap[2k]   = x[2k+1]
            snrt_dma_start_2d(xswap + 2, x_in, 2, 4, 4,
