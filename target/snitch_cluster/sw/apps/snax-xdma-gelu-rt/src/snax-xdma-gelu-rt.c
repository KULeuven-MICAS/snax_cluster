...
//
// RUNTIME-PRECISION xDMA SiLU: out = gelu(x) = x * sigmoid(x), run end-to-end at
// FP16 AND FP8 on the SAME xDMA netlist, the precision chosen purely by the `fmt`
// CSR field. This is the runtime-precision counterpart of snax-xdma-gelu: one set
...
// no host reciprocal, no fp32out. A single StreamMapRt(GELU) pass does it:
//
//   T1  gelu : StreamMapRt(a=1, b=0, func=GELU)   x -> out   (= x*sigmoid(x))
//
// GELU routes the affine result (a*x+b) through the merged FpActivation core (an
...
#define ACT_EXP 1u
#define ACT_SILU 2u
#define ACT_GELU 3u  // StreamMapRt func CSR: 0=LINEAR,1=EXP,2=SILU,3=GELU
...
// One gelu pass at the given runtime precision. All buffers are TCDM. Writes
...
static int run_chain_rt(uint32_t fmt, uint32_t beats, uint8_t* x_in,
    ...
    // T1: out = gelu(x). a=1.0, b=0, func=GELU. Single unary pass — no reduce, no
    // scalar glue (StreamElementwiseRt idx 7 / StreamReduceRt idx 8 stay disabled
    // = bypass).
    uint32_t csr_gelu[3] = {0x3F800000u /*a=1.0f*/, 0u, ACT_GELU | (fmt << 2)};
    ok &= (snax_xdma_enable_src_ext(READER_EXT_STREAMMAPRT, csr_gelu) == 0);
...
// FP16-ULP integer compare. gelu is SIGNED (small negative for x<0), but gelu is
...
static uint32_t check_fp16(uint16_t* out, const uint16_t* gold, uint32_t n,
    ...
    *checked = 0;
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (ulp > tol) {
            if (mism < 6)
                printf("[GeluRt] fp16 mism[%u]: got %04x gold %04x (%u ulp)\n",
                       i, out[i], gold[i], ulp);
            mism++;
        }
    }
...
// e5m2-code integer compare. Same signed-monotonic argument as check_fp16: gelu
...
static uint32_t check_fp8(uint8_t* out, const uint8_t* gold, uint32_t n,
    ...
    *checked = 0;
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (ulp > tol) {
            if (mism < 6)
                printf("[GeluRt] fp8 mism[%u]: got %02x gold %02x (%u ulp)\n",
                       i, out[i], gold[i], ulp);
            mism++;
        }
    }
...
int main() {
    ...
    if (snrt_is_dm_core()) {
        ...
        uint32_t n = gelu_n;
        printf("[GeluRt] N=%u : one netlist, FP16 then FP8 by fmt CSR\n", n);

        // ---------- FP16 (32 elems/beat) ----------
        {
            uint32_t beats = gelu_beats_fp16;
            uint32_t row_bytes = beats * XDMA_BEAT_BYTES;
            uint8_t* x_in = (uint8_t*)base;
            uint8_t* out_buf = x_in + row_bytes;

            snrt_dma_start_1d(x_in, gelu_input_fp16, row_bytes);
            snrt_dma_wait_all();
            ...
            if (!ok || c1 == 0xFFFFFFFFu) {
                printf("[GeluRt] FP16 task setup failed\n");
                return 1;
            }
            uint32_t worst, checked;
            uint32_t mism = check_fp16((uint16_t*)out_buf, gelu_golden_fp16, n,
                                       4, &worst, &checked);
            printf("[GeluRt] FP16 beats=%u cycles: gelu=%u | cold=%u warm=%u\n",
                   beats, c1, cold, warm);
            printf("[GeluRt] FP16 significant=%u/%u worst ULP=%u : %s\n",
        ...
        {
            uint32_t beats = gelu_beats_fp8;
            uint32_t row_bytes = beats * XDMA_BEAT_BYTES;
            uint8_t* x_in = (uint8_t*)base;
            uint8_t* out_buf = x_in + row_bytes;

            snrt_dma_start_1d(x_in, gelu_input_fp8, row_bytes);
            snrt_dma_wait_all();
            ...
            if (!ok || c1 == 0xFFFFFFFFu) {
                printf("[GeluRt] FP8 task setup failed\n");
                return 1;
            }
            uint32_t worst, checked;
            uint32_t mism = check_fp8((uint8_t*)out_buf, gelu_golden_fp8, n, 2,
                                      &worst, &checked);
            printf("[GeluRt] FP8  beats=%u cycles: gelu=%u | cold=%u warm=%u\n",
                   beats, c1, cold, warm);
            printf("[GeluRt] FP8  significant=%u/%u worst e5m2 ULP=%u : %s\n",
                   checked, n, worst, mism == 0 ? "PASS (<=2 e5m2)" : "FAIL");
            if (mism) err++;
        }

        if (!err)
            printf("[GeluRt] ALL PASS — one netlist, gelu at FP16 & FP8 by runtime fmt\n");
    }
    return err != 0;
}
