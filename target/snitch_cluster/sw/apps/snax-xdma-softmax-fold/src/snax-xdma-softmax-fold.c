...
//
// xDMA FP16 multi-row softmax, A/B: per row, out[r,:] = exp(x[r,:]-max[r]) / Sexp[r].
// Same kernel as snax-xdma-softmax-multirow (same tensor, same golden); this app runs the
// DEPLOYED path and a SOFTWARE-ONLY optimized path back to back on the same data.
//
// The per-row scalars (max[r], 1/Sexp[r]) cannot be held constant across a row by the
// reader AGU -- a single affine address stream cannot keep one operand's address fixed
// while the other walks the row -- so they must be materialized as full [rows,D] operand
// tensors. That is unavoidable without an RTL change. What IS avoidable is materializing
// them with the DM core, and paying a separate pass for the row-sum and for the quantize:
//
//   A (deployed)   T1 StreamReduce(MAX)                    x    -> max[rows]
//                  A1 DM-core broadcast loop               -max[r] -> [rows,D]   (rows*D)
//                  A2 StreamElementwise(ADD)               x (.) -max -> xs
//                  A3 StreamMap(EXP)                       xs   -> expb
//                  A4 StreamReduce(ADD)                    expb -> sum[rows]   [re-reads expb]
//                  A5 DM-core broadcast loop               1/sum[r] -> [rows,D] (rows*D)
//                  A6 StreamElementwise(MUL)               expb (.) recip -> out
//                  A7 Fp16ToInt8 (separate pass)           out  -> int8        [re-reads out]
//
//   B (this app)   T1 StreamReduce(MAX)                    x    -> max[rows]
//                  B0 DM-core: sign-flip the SPLATTED max beat, one beat per row
...
//                      row-sum as a trailing beat, so A3+A4 collapse -- expb is never
//                      re-read. Output is beats+1 beats per row: a PADDED row layout.)
//                  B4 DM-core splat of 1/Sexp[r]           (rows*16 word stores)
//                  B5 xDMA BROADCAST pass, stride 0, written at the PADDED row stride so
//                     it lines up with expb's padded rows (the EW interleave needs both
...
//                      3D reader: {operand, beat, row} skips each row's trailing sum beat.)
//
// The subtract cannot fuse with the exp: the reader extensions are chained in the hjson's
// list order (... StreamMap, StreamReduce, StreamElementwise, Fp16ToInt8), so an
// elementwise op can never precede a map in one task. B keeps A2 as its own pass.
//
// Cross-checks (all bit-exact, all in this app):
//   out_b   == out_a          -> the whole B chain reproduces the deployed FP16 result
//   tap sum == A4's sum       -> the multi-row TAP scalar equals the standalone reduce
//   q_fused == q_sep          -> chaining the quant onto the norm pass is bit-identical
// and out_a is checked against the FP64-derived golden (<=4 FP16 ULP), as in the deployed app.
//
// inv_sum[] is DMA'd to L1 up front and both core loops read it from there: in data.h it
// lives in L3, and a per-row DRAM load would tax both paths for reasons that have nothing
// to do with the kernel.
//
// MEASURED (vsim, warm, L1<->L1). A's xDMA task cycles match snax-xdma-softmax-multirow
// exactly at 4x64 (max 197, sub 144, exp 86, sum 197, norm 144), so A is the deployed
// kernel, not a strawman. The win scales with D: A's two broadcast loops are 2*rows*D core
// stores (~4 cc each), B's two splats are rows*16 each -- independent of D.
//
...
//
// The TAP fusion also shrinks the datapath itself: B3 = 242 cc vs A3+A4 = 86+197 = 283 cc
// at 4x64, and it removes a whole task setup (~600 cc of CSR orchestration).
//
// EXAMPLE (params.hjson default: rows=16, D=256 -> beats=8; FP16, all L1-local)
//   row_bytes = beats*64 = 512, rows_bytes = 8 KiB, padded row = (beats+1)*64 = 576.
...
#define RED_TAP 0x100u  // StreamReduce op CSR bit[8]: pass the row through + trailing scalar
...
int main() {
    ...
    if (snrt_is_dm_core()) {
        ...
        uint32_t pad_row_bytes = (beats + 1) * XDMA_BEAT_BYTES;  // + the trailing sum beat
        ...
        uint8_t* neg_beat = inv_l1 + XDMA_BEAT_BYTES;  // B0: -max[r], one beat per row
        uint8_t* rcp_beat = neg_beat + scal_bytes;     // B4: 1/Sexp[r], one beat per row
        // ORDER MATTERS. The elementwise interleave addresses operand 1 as
        // operand0_base + delta, and the AGU adds `delta` as an UNSIGNED stride -- a
        // broadcast buffer placed BELOW its data operand makes the delta wrap and the
        // reader walks off outside the TCDM (it returns X, the task still "completes",
        // and X lands in the result). So every broadcast buffer must sit ABOVE every
        // data operand it is interleaved with: bc_a above x_in AND above expb_a; bc_b
        // above x_in AND above expb_pad.
        ...
        uint8_t* expb_pad = q_sep + rows_bytes / 2;    // B3 out: [rows, beats+1] beats
        ...
        // ---- AGU shapes ------------------------------------------------------------
        uint32_t red_str[2] = {XDMA_BEAT_BYTES, row_bytes};  // reduce reader {beats, rows}
        uint32_t red_bnd[2] = {beats, rows};
        uint32_t w_rows_str[1] = {XDMA_BEAT_BYTES};
        uint32_t w_rows_bnd[1] = {rows};
        uint32_t flat_str[1] = {XDMA_BEAT_BYTES};   // 1D over the whole [rows,D] tensor
        uint32_t flat_bnd[1] = {all_beats};
        uint32_t q_bnd[1] = {all_beats / 2};        // Fp16ToInt8: 2 FP16 beats -> 1 INT8
        ...
        // B5 writer: the bcast lands at the PADDED row stride (only the beats data slots),
        // so expb_pad and bc_b share a row stride and the B6 interleave delta is constant.
        uint32_t padw_str[2] = {XDMA_BEAT_BYTES, pad_row_bytes};
        uint32_t padw_bnd[2] = {beats, rows};
        // B6 reader: 3D {operand, beat, row} over the padded rows -- the trailing sum beat
        // of each row is simply never addressed.
        uint32_t ew_b_mul_str[3] = {(uint32_t)(bc_b - expb_pad), XDMA_BEAT_BYTES,
                                    pad_row_bytes};
        uint32_t ew_b_mul_bnd[3] = {2, beats, rows};

        // Guard the unsigned-delta rule above: an interleave whose operand-1 region sits
        // below its operand-0 region silently reads outside the TCDM.
        if (bc_a <= x_in || bc_a <= expb_a || bc_b <= x_in || bc_b <= expb_pad) {
        ...
        uint32_t d_max = 0, d_sub = 0, d_exp = 0, d_sum = 0, d_norm = 0, d_qsep = 0;
        ...
        for (int it = 0; it < 2; it++) {
            ...
            ok &= (snax_xdma_memcpy_nd_fast(x_in, max_buf, 8, 8, 2, red_str, red_bnd, 1,
                                            w_rows_str, w_rows_bnd, 0xFFFFFFFF,
            ...
            // ================= PATH A: deployed =================
            // A1: DM-core broadcast of -max[r] (FP16 sign flip) -- rows*D stores.
            t0 = snrt_mcycle();
            ...
            ok &= (snax_xdma_memcpy_nd_fast(x_in, xs_buf, 8, 8, 2, ew_a_add_str, ew_bnd,
                                            1, flat_str, flat_bnd, 0xFFFFFFFF, 0xFFFFFFFF,
                                            0xFFFFFFFF) == 0);
            d_sub = run_task();
            ...
            ok &= (snax_xdma_memcpy_nd_fast(xs_buf, expb_a, 8, 8, 1, flat_str, flat_bnd,
                                            1, flat_str, flat_bnd, 0xFFFFFFFF, 0xFFFFFFFF,
                                            0xFFFFFFFF) == 0);
            d_exp = run_task();
            ...
            ok &= (snax_xdma_memcpy_nd_fast(expb_a, sum_buf, 8, 8, 2, red_str, red_bnd, 1,
                                            w_rows_str, w_rows_bnd, 0xFFFFFFFF,
                                            0xFFFFFFFF, 0xFFFFFFFF) == 0);
            d_sum = run_task();
            snax_xdma_disable_src_ext(READER_EXT_STREAMREDUCE);
            ...
            ok &= (snax_xdma_memcpy_nd_fast(expb_a, out_a, 8, 8, 2, ew_a_mul_str, ew_bnd,
                                            1, flat_str, flat_bnd, 0xFFFFFFFF, 0xFFFFFFFF,
                                            0xFFFFFFFF) == 0);
            d_norm = run_task();
            snax_xdma_disable_src_ext(READER_EXT_STREAMELEMENTWISE);
            ...
            ok &= (snax_xdma_memcpy_nd_fast(out_a, q_sep, 8, 8, 1, flat_str, flat_bnd, 1,
            ...
            // B0: sign-flip the already-SPLATTED max beat into -max, one beat per row.
            t0 = snrt_mcycle();
            for (uint32_t r = 0; r < rows; r++)
            ...
            t0 = snrt_mcycle();
            ok &= (snax_xdma_memcpy_nd_fast(neg_beat, bc_b, 8, 8, 2, bc_str, bc_bnd, 1,
            ...
            t0 = snrt_mcycle();
            ok &= (snax_xdma_enable_src_ext(READER_EXT_STREAMELEMENTWISE, csr_add) == 0);
            ok &= (snax_xdma_memcpy_nd_fast(x_in, xs_buf, 8, 8, 2, ew_b_add_str, ew_bnd,
                                            1, flat_str, flat_bnd, 0xFFFFFFFF, 0xFFFFFFFF,
                                            0xFFFFFFFF) == 0);
            (void)run_task();
            snax_xdma_disable_src_ext(READER_EXT_STREAMELEMENTWISE);
            ...
            // StreamReduce(ADD, TAP). Writes beats+1 beats per row (padded rows).
            t0 = snrt_mcycle();
            ok &= (snax_xdma_enable_src_ext(READER_EXT_STREAMMAP, csr_exp) == 0);
            ok &= (snax_xdma_enable_src_ext(READER_EXT_STREAMREDUCE, csr_sum_tap) == 0);
            ok &= (snax_xdma_memcpy_nd_fast(xs_buf, expb_pad, 8, 8, 1, flat_str, flat_bnd,
                                            2, tap_w_str, tap_w_bnd, 0xFFFFFFFF,
                                            0xFFFFFFFF, 0xFFFFFFFF) == 0);
            d_expsum = run_task();
            snax_xdma_disable_src_ext(READER_EXT_STREAMREDUCE);
            snax_xdma_disable_src_ext(READER_EXT_STREAMMAP);
            ...
            t0 = snrt_mcycle();
            ok &= (snax_xdma_memcpy_nd_fast(rcp_beat, bc_b, 8, 8, 2, bc_str, bc_bnd, 2,
                                            padw_str, padw_bnd, 0xFFFFFFFF, 0xFFFFFFFF,
                                            0xFFFFFFFF) == 0);
            (void)run_task();
            t1 = snrt_mcycle();
            t_bc2 = t1 - t0;

            // B6: out = expb * recip, with Fp16ToInt8 CHAINED (int8 out, no re-read).
            t0 = snrt_mcycle();
            ok &= (snax_xdma_enable_src_ext(READER_EXT_STREAMELEMENTWISE, csr_mul) == 0);
            ok &= (snax_xdma_enable_src_ext(READER_EXT_FP16TOINT8, csr_q) == 0);
            ok &= (snax_xdma_memcpy_nd_fast(expb_pad, q_fused, 8, 8, 3, ew_b_mul_str,
                                            ew_b_mul_bnd, 1, flat_str, q_bnd, 0xFFFFFFFF,
                                            0xFFFFFFFF, 0xFFFFFFFF) == 0);
            d_normq = run_task();
            snax_xdma_disable_src_ext(READER_EXT_FP16TOINT8);
            snax_xdma_disable_src_ext(READER_EXT_STREAMELEMENTWISE);
            ...
            // B6': the same norm WITHOUT the chained quant -- path B's FP16-out variant,
            // and the buffer that lets out_b be compared to out_a bit-exactly.
            t0 = snrt_mcycle();
            ok &= (snax_xdma_enable_src_ext(READER_EXT_STREAMELEMENTWISE, csr_mul) == 0);
            ok &= (snax_xdma_memcpy_nd_fast(expb_pad, out_b, 8, 8, 3, ew_b_mul_str,
                                            ew_b_mul_bnd, 1, flat_str, flat_bnd,
        ...
        uint32_t a_fp16 = t_max + t_negloop + t_sub + t_exp + t_sum + t_rcploop + t_norm;
        uint32_t b_fp16 = t_max + t_neg0 + t_bc1 + t_sub_b + t_expsum + t_rcp0 + t_bc2 +
                          t_norm_b;
        uint32_t a_int8 = a_fp16 + t_qsep;
        uint32_t b_int8 = b_fp16 - t_norm_b + t_normq;

        printf("[SmFold] --- stage cycles (warm; wall = CSR cfg + start + wait) ---\n");
        ...
        printf("[SmFold] B3 EXP-||>reduce(TAP)  wall=%u  datapath=%u  (fuses A3+A4)\n",
               t_expsum, d_expsum);
        ...
        for (uint32_t i = 0; i < rows * d; i++) {
            ...
            if (ulp > 4) {
                if (mism < 6)
                    printf("[SmFold] mismatch[%u] (row %u): got %04x golden %04x\n", i,
                           i / d, oa[i], smf_out_golden[i]);
                mism++;
            }
        }
        ...
        // The TAP scalar (trailing beat of each padded row) must equal the standalone
        // reduce's per-row sum.
        uint32_t bad = 0;
        ...
        // Chaining the quant onto the norm pass must equal a separate quant pass.
