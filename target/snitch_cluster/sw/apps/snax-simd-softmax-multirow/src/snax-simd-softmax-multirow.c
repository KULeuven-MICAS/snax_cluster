...
//
// SIMD FP16 multi-row softmax: per row, out[r,:] = exp(x[r,:]-max[r]) /
// Sexp[r]. The PER-ROW form the llama3 attention needs -- S independent rows
// softmaxed in one dispatch. Each row has its own max and 1/Sexp, which
// StreamMap's single scalar can't express; the multi-row pipeline uses a host
// scalar-broadcast + StreamElementwise:
//
//   T1 max    : StreamReduce(MAX, rows)               x       -> max[rows]
//   [xDMA, multi-row] bcast     : -max[r] -> negmax_bcast[r,:]   (FP16 sign
//   flip, integer)            [DM core] T2 sub    : StreamElementwise(ADD) x
//   (.) negmax -> xs[rows,D]      [xDMA, element-wise] T3 exp    :
//   StreamMap(EXP, a=1, b=0)             xs      -> expb[rows,D]         [xDMA,
//   element-wise] T4 sum    : StreamReduce(ADD, rows)              expb    ->
//   sum[rows]            [xDMA, multi-row] bcast     : 1/sum[r] ->
//   recip_bcast[r,:]   (recip precomputed, integer copy)    [DM core] T5 norm
//   : StreamElementwise(MUL)              expb (.) recip -> out[rows,D] [xDMA,
//   element-wise]
//
// LANE DATAFLOW (FP16 transport = 32 lanes per 512-b beat). Only the two
// reduces change the beat shape; the sew/smap passes are clean element-wise
// over the flat rows*beats stream:
//   T1 StreamReduce(MAX)  per row N beats -> 1 beat: 32 lanes collapse to
//   max[r], SPLATTED (read lane 0). bcast: -max[r] -> negmax_bcast[r,:]  (FP16
//   sign flip ^0x8000, replicated across the D lanes). T2
//   StreamElementwise(ADD)  2 beats -> 1 beat: xs[i]   = x[i] + (-max[i]) (full
//   32 lanes) T3 StreamMap(EXP)          1 beat  -> 1 beat: expb[i] =
//   exp(xs[i])              (full 32 lanes) T4 StreamReduce(ADD)  per row N
//   beats -> 1 beat: 32 lanes collapse to Sexp[r], SPLATTED. bcast: 1/sum[r] ->
//   recip_bcast[r,:]  (replicated across the D lanes). T5
//   StreamElementwise(MUL)  2 beats -> 1 beat: out[i]  = expb[i] * recip[i]
//   (full 32 lanes) (each sew reads its two operands interleaved -> 2x the
//   beats it writes; the reduces are rows*N -> rows.)
//
// The DM core (rv32ima, no FPU) does -max as an integer sign flip; the
// reciprocal is precomputed in data.h (a host core does it). Checks out vs
// smr_out_golden, which mirrors the FP16 chain.
//
...
//   The 5 xDMA passes thread 6 intermediate [rows,D]/[rows] buffers (max,
//   negmax, xs, expb, sum, recip).
//
//   L1 memory map (byte offset from cluster base):
//     +0     x          512 B   [4,64] input
//     +512   max        256 B   [4] MAX scalars (splatted beats; T1)
//     +768   negmax_bc  512 B   [4,64] -max[r] broadcast (DM core; T2 operand
//     1) +1280  xs         512 B   [4,64] x-max          (T2 out / T3 in) +1792
//     expb       512 B   [4,64] exp(x-max)     (T3 out / T4,T5 in) +2304  sum
//     256 B   [4] Sexp scalars      (T4) +2560  recip_bc   512 B   [4,64]
//     1/Sexp[r] bcast (DM core; T5 operand 1) +3072  out        512 B   [4,64]
...
int main() {
    ...
    if (snax_is_simd_core()) {
        ...
        for (uint32_t r = 0; r < rows; r++) {
            uint16_t neg = (uint16_t)(mx[r * FP16_PER_BEAT] ^
                                      0x8000u);  // low lane of splatted beat r
        ...
        // Per-row FP16-ULP check on the significant outputs. Tolerance 8: the
        // multi-row chain narrows to FP16 once more than the single-row fused
        // path (xs and recip both narrowed), and the HW exp is a LUT vs the
        // np.exp golden -- a broken row/AGU would be off by hundreds of ULP,
        // not <=8.
        uint16_t* out_h = (uint16_t*)out_buf;
        uint32_t mism = 0, worst = 0, checked = 0;
        for (uint32_t i = 0; i < rows * d; i++) {
            if ((smr_out_golden[i] & 0x7C00u) == 0)
            ...
            if (ulp > 8) {
                if (mism < 6)
                    printf(
                        "[SmMR] mismatch[%u] (row %u): got %04x golden %04x "
                        "(%u ulp)\n",
                        i, i / d, out_h[i], smr_out_golden[i], ulp);
                mism++;
            }
        }
        ...
        if (mism == 0)
            printf(
                "[SmMR] PASS (%u significant elements over %u rows, <=8 ULP)\n",
                checked, rows);
        else {
