...
//
// SIMD FP16 rmsnorm: out = x * rsqrt(mean(x^2)), mean = sum(x^2)/N, eps = 0.
// The SIMD-wide work runs on the xDMA; the scalar rsqrt runs on the host (a
// single 1/sqrt only uses one lane, so it wastes the datapath -- StreamScalar
// and StreamMap.operandMode were removed for it):
//
//   T1  sum(x^2)  : StreamReduce(SUMSQ)                      x -> ssq [xDMA]
//   host          : inv_rms = 1/sqrt(ssq/N) [host, ~110 cc] T2  scale     :
//   StreamMap(a=inv_rms, b=0, func=LINEAR)   x -> out           [xDMA]
//
// LANE DATAFLOW (FP16 transport = 32 lanes per 512-b beat):
//   T1 StreamReduce(SUMSQ)  N beats -> 1 beat (the row's 32 lanes COLLAPSE to a
//   scalar; not 512->512):
//     in beat 0..N-1 [ x0 .. x31 ] -> per-lane FP32 partials accumulate, then a
//     horizontal tree folds them to s = sum(x^2) over the row;  out [ s s ... s
//     ] SPLATTED across all 32 lanes (read lane 0).
//   T2 StreamMap(LINEAR)    1 beat -> 1 beat, all 32 lanes:  out[i] =
//   inv_rms*x[i] + 0   (clean 512->512) Fp16ToInt8 (fused quant): packs 2 FP16
//   beats -> 1 INT8 beat (writer drains beats/2, even count).
//
// The DM core (rv32ima, no FPU) can't do the rsqrt; a host core does it. This
// standalone test precomputes inv_rms in data.h (rmsnorm_inv_rms; mean =
// sum(x^2)/N is exact for N=2^log2n) and checks the runtime SUMSQ against
// rmsnorm_ssq_golden.
//
// Performance (FP16, vsim, L1<->L1; xDMA part) on the native-Chisel FP RTL
// (FpAdd/FpMul/FpFma, fpPipe=1 cut) + time-mux computeLanes (StreamReduce=4,
// StreamMap=2, Fp16ToInt8=8). "+host" adds the estimated host rsqrt
// (~110 cc, from the fp16_sqrt + fp16_reciprocal op-LUTs at n=1). host = a
// single host vector core, full FP32 rmsnorm. Outputs match the FP64 golden to
// <=4 ULP (worst FP16 ULP=1). The CSR orchestration is a fixed ~0.9k cc (3 task
// setups); the datapath scales with N (CSR-bound at small N, datapath-bound
// N>=1k).
//
//   N      beats   xDMA warm   +host est   cold    host(full)   warm
//   speedup(+host)
//   ----   -----   ---------   ---------   -----   ----------
//   ------------------- 64     2       1,084       ~1,194      1,748 2,962 2.5x
//   256    8       1,480       ~1,590      2,144   11,239       7.1x
//   1024   32      3,060       ~3,170      3,710   44,463       14.0x
...
int main() {
    ...
    if (snax_is_simd_core()) {
        ...
        snax_stage_1d(x_in, rmsnorm_input, row_bytes);

        // The AGU shape (2D [beats,1] src reading x_in, 1D dst) is identical
        // for both tasks and across rows, so it is programmed ONCE (full
        ...
        int ok = 1;
        // Run twice: iter 0 = cold (one-time AGU shape setup + icache warming);
        // iter 1 = warm / steady-state.
        for (int iter = 0; iter < 2; iter++) {
            ...
            // rmsnorm_inv_rms). Validate T1's reduce.
            if (iter == 1) {
                ...
                if (u > 2)
                    printf(
                        "[Rmsnorm] WARN runtime ssq %04x vs golden %04x (%u "
                        "ulp)\n",
                        ((uint16_t*)ssq_buf)[0], (uint16_t)rmsnorm_ssq_golden,
                        u);
            }

            // T2: out = x * inv_rms. Plain LINEAR multiply by the host-provided
        ...
        printf(
            "[Rmsnorm] cycles: sumsq=%u norm=%u norm+quant=%u (quant "
            "marginal=%u)\n",
            c1, c2, cq, cq - c2);
        printf("[Rmsnorm] full latency: cold=%u warm=%u cycles\n", lat_cold,
               lat_warm);

        // Integer FP16-ULP check on the significant (signed) outputs; skip the
        // negligible subnormal/zero golden (FTZ tail). The DM core has no FPU,
        // so the compare stays integer (monotonic-key ULP).
        uint16_t* out_h = (uint16_t*)out_buf;
        uint32_t mism = 0, worst = 0, checked = 0;
        for (uint32_t i = 0; i < rmsnorm_n; i++) {
            if ((rmsnorm_golden[i] & 0x7C00u) == 0)
            ...
            if (ulp > 4) {
                if (mism < 6)
                    printf(
                        "[Rmsnorm] mismatch[%u]: got %04x golden %04x (%u "
                        "ulp)\n",
        ...
        // Quantize check: fused int8 out vs golden quant(rmsnorm_golden).
