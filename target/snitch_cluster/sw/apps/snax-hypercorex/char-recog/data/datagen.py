...
def main():
    ...
    train_code_list, train_control_code_list = compile_hypercorex_asm(train_asm_path)
    test_code_list, test_control_code_list = compile_hypercorex_asm(test_asm_path)
    ...
    num_test_items_str = format_scalar_definition('uint32_t', 'num_test_items', NUM_ITEMS_TO_TEST)
    num_features_str = format_scalar_definition('uint32_t', 'num_features', TOTAL_PIXEL_FEATURES)
    train_inst_str = format_vector_definition('uint32_t', 'train_inst_code', train_code_list)
    test_inst_str = format_vector_definition('uint32_t', 'test_inst_code', test_code_list)
    im_seed_list_str = format_vector_definition('uint32_t', 'im_seed_list', im_seed_list)
    char_data_str = format_vector_definition('uint64_t', 'char_data', flat_char_dataset)

    # Preparing string to load
    f_str = '\n\n'.join([num_classes_str, num_test_items_str, num_features_str, \
                         train_inst_str, test_inst_str, im_seed_list_str, char_data_str])
    f_str += '\n'
