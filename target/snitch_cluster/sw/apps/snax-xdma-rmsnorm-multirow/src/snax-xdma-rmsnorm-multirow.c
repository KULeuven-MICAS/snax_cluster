...
//
// xDMA FP16 multi-row RMSNorm: per row, out[r,:] = x[r,:] * inv_rms[r],
// inv_rms[r]=1/sqrt(mean(x[r,:]^2)). This is the PER-ROW form the llama3 layer
// needs -- S independent rows scaled in one dispatch. The single-row app
// applies one StreamMap scalar to the whole row, which can't vary per row; the
// multi-row pipeline is instead:
//
//   T1  Sx^2   : StreamReduce(SUMSQ, rows)            x[rows,D] -> ssq[rows]
//   [xDMA, multi-row] bcast      : inv_rms[r] -> inv_bcast[r,:] [DM core,
//   integer] T2  scale  : StreamElementwise(MUL)              x (.) inv_bcast
//   -> out[rows,D] [xDMA, element-wise]
//
// LANE DATAFLOW (FP16 transport = 32 lanes per 512-b beat):
//   T1 StreamReduce(SUMSQ, rows)  per row N beats -> 1 beat: the row's 32 lanes
//   COLLAPSE to a scalar
//      ssq[r]=sum(x[r,:]^2) SPLATTED across all 32 lanes (read lane 0). rows*N
//      beats in -> rows beats out.
//   bcast (DM core): inv_rms[r] written to ALL D lanes of inv_bcast[r,:]
//   ([rows,D] buffer; integer copy). T2 StreamElementwise(MUL)     2 beats -> 1
//   beat: out[i] = x[i] * inv_bcast[i], full 32 lanes. The
//      reader interleaves {x_beat, inv_bcast_beat} over rows*beats beats (reads
//      2x what the writer emits).
//
// The DM core (rv32ima, no FPU) can't do the rsqrt; a host core does it. Here
// inv_rms[rows] is precomputed in data.h (derived from the FP16-narrowed
// per-row Sx^2 so it matches the runtime reduce) and the DM core only
// replicates it across the row. Checks out vs rmsmr_out_golden.
//
// EXAMPLE (params.hjson default: rows=4, D=64 -> beats=D/32=2; FP16, all
// L1-local)
//   in   x    [rows=4, D=64] fp16  = 4*128 = 512 B
//   out  out  [rows=4, D=64] fp16  = 4*128 = 512 B   (per-row out[r,:] = x[r,:]
//   * inv_rms[r]) intermediates: ssq[4] (one splatted 64-B beat/row),
//   inv_bcast[4,64] (inv_rms[r] replicated).
//
//   L1 memory map (byte offset from cluster base):
//     +0     x          512 B   [4,64] input
//     +512   ssq        256 B   [4] SUMSQ scalars (splatted beats; T1 writer =
//     4 beats) +768   inv_bcast  512 B   [4,64] inv_rms[r] broadcast (DM-core
//     fill; T2 operand 1) +1280  out        512 B   [4,64] result (T2 writer)
//     total  1792 B
...
int main() {
    ...
    if (snrt_is_dm_core()) {
        ...
        // T2: out = x (.) inv_bcast. StreamElementwise(MUL), operandCount=2
        ...
        // Per-row integer FP16-ULP check on the significant outputs (skip
        // subnormal/zero golden tail).
        ...
        for (uint32_t i = 0; i < rows * d; i++) {
            ...
            checked++;
            uint32_t o = fp16_mono(out_h[i]),
                     g = fp16_mono(rmsmr_out_golden[i]);
            uint32_t ulp = (o > g) ? (o - g) : (g - o);
            if (ulp > worst) worst = ulp;
            if (ulp > 4) {
                if (mism < 6)
                    printf(
                        "[RmsMR] mismatch[%u] (row %u): got %04x golden %04x "
                        "(%u ulp)\n",
                        i, i / d, out_h[i], rmsmr_out_golden[i], ulp);
                mism++;
            }
        }
        ...
        if (mism == 0)
            printf(
                "[RmsMR] PASS (%u significant elements over %u rows, <=4 "
                "ULP)\n",
                checked, rows);
        else {
