...
//
// Standalone multi-row xDMA StreamReduce test. Drives S independent per-row
// reductions in ONE dispatch (the per-row form RMSNorm/Softmax need): a 2D
// reader {beats inner, rows outer} feeds StreamReduce, whose counter wraps
// every `beats` beats and re-inits the per-lane accumulators, emitting one
// splatted scalar beat per row; a 1D writer drains `rows` beats. No RTL change
// vs the single-row reduce -- only the AGU shape ({beats,rows} reader, {rows}
// writer) differs.
//
// It also exercises the mem_wide_narrow_mux DMAWriteDataCorrect assert: with
// the input fully initialized, the reduce writes defined (non-X) data, so the
// assert (gated on q.write) stays silent.
//
// LANE DATAFLOW -- StreamReduce (N beats in -> 1 beat out; a row's 32 lanes
// collapse to one scalar). This is NOT a 512b-in/512b-out element-wise op: the
// output beat carries ONE useful value, not 32.
//   in  beat k=0..N-1 : [ l0  l1  ... l31 ]   32 FP16 lanes/beat; N = beats per
//   row (= D/32)
//        the 32 per-lane FP32 partials accumulate down the N beats; a
//        horizontal reduce-tree then folds them into ONE scalar s = max | sum |
//        sum-of-squares over the whole D-element row.
//   out beat          : [ s   s   ...  s  ]   s SPLATTED across all 32 lanes (1
//   useful + 31 copies) Read lane 0 only (stride 32 lanes = one 64-B beat). N
//   input beats -> 1 output beat per row.
//
// EXAMPLE (params.hjson default: rows=4, D=64 -> beats=D/32=2; FP16, all
// L1-local)
//   in   x    [rows=4, D=64] fp16  = 4*128 = 512 B
//   out  one scalar PER ROW per op -> 4 splatted 64-B beats = 256 B/op (useful
//   payload 4 fp16 = 8 B/op)
//
...
//     total  1280 B
//
// Performance (FP16, vsim, L1<->L1) on the native-Chisel FP RTL (StreamReduce
// FpAdd/FpMul, fpPipe=1 cut, treePipe=1 reduce-tree cut; computeLanes=4).
// Default rows=4, D=64 (2 beats/row): each op (SUMSQ/MAX/ADD, and the fp32out
// big16/big32 paths) measures 131 cycles. Outputs match the FP64 golden to 0
...
// FP32 bits -> monotonic ordering key: flip the sign bit for positives, invert
// all bits for negatives. For two same-sign values |key(a)-key(b)| is their
// FP32-ULP distance (no FPU needed on the DM core).
static inline uint32_t fp32_mono(uint32_t b) {
    return (b & 0x80000000u) ? (~b) : (b | 0x80000000u);
}

// Exact integer FP16 -> FP32 bit widen (normals/inf/nan; subnormals
// normalized). Lets the no-FPU DM core compare an fp16-out scalar against the
// true FP32 SUMSQ to show the fp16 path can't represent it.
...
// index r*FP16_PER_BEAT) against the golden. Returns the number of rows beyond
// tolerance.
static uint32_t check_rows(const char* name, uint16_t* out,
                           const uint16_t* golden, uint32_t rows) {
...
// Per-row FP32 scalars (one splatted beat each: lane 0 is at uint32 index
// r*FP32_PER_BEAT). Flags a row that is non-finite (inf/NaN exponent) or beyond
// an FP32-ULP tolerance of the golden. Returns bad rows.
static uint32_t check_rows_fp32(const char* name, const uint32_t* out,
                                const uint32_t* golden, uint32_t rows) {
    uint32_t worst = 0, bad = 0;
    for (uint32_t r = 0; r < rows; r++) {
        ...
        if (ulp > worst) worst = ulp;
        if (exp == 0xFFu ||
            ulp > 0x10000u) {  // non-finite, or > ~0.8% (tree-order vs
                               // sequential FP32)
            if (bad < 6)
                printf(
                    "[ReduceMR] %s row %u: got %08x golden %08x (%u "
                    "fp32-ulp%s)\n",
                    name, r, got, golden[r], ulp,
                    exp == 0xFFu ? ", NON-FINITE" : "");
            bad++;
        }
    }
...
// Show the default fp16-out path is unusable for a SUMSQ this large: widen each
// fp16-out scalar to fp32 and report its FP32-ULP distance from the true SUMSQ.
// inf (0x7c00) or wrapped garbage both land millions of ULPs off; a row counts
// as "broken" when off by > ~25% of the golden. Returns broken rows.
static uint32_t count_fp16_broken(const char* name, const uint16_t* out,
                                  const uint32_t* golden_f32, uint32_t rows) {
    uint32_t broken = 0;
    for (uint32_t r = 0; r < rows; r++) {
        uint16_t got = out[r * FP16_PER_BEAT];
    ...
    printf(
        "[ReduceMR] %s: %u/%u rows unrepresentable in fp16 (fp32out fixes "
        "this)\n",
        name, broken, rows);
    return broken;
}
...
int main() {
    ...
    if (snax_is_simd_core()) {
        ...
        printf("[ReduceMR] cycles: sumsq=%u max=%u add=%u big16=%u big32=%u\n",
               c_ssq, c_max, c_add, c_big16, c_big32);

        err += (int)check_rows("SUMSQ", (uint16_t*)ssq_buf, reduce_ssq_golden,
                               rows);
        err +=
            (int)check_rows("MAX", (uint16_t*)max_buf, reduce_max_golden, rows);
        err +=
            (int)check_rows("ADD", (uint16_t*)sum_buf, reduce_sum_golden, rows);

        // The fp16-out scalar can't represent a SUMSQ this large (evidence of
        // the bug); the fp32-out scalar must be the true value (the fix). Only
        // the fp32-out path is pass/fail; the fp16 breakage is logged evidence
        // (expected to be all rows -- it demonstrates the GAP-2 overflow the
        // fp32out flag fixes).
        count_fp16_broken("SUMSQ-big-fp16", (uint16_t*)ssq16_big_buf,
                          reduce_ssq_big_f32_golden, rows);
        err += (int)check_rows_fp32("SUMSQ-big-fp32", (uint32_t*)ssq32_big_buf,
                                    reduce_ssq_big_f32_golden, rows);

        if (err == 0)
            printf(
                "[ReduceMR] PASS (all rows <=4 ULP; fp32out SUMSQ recovers the "
                "overflowing reduction)\n");
        else
