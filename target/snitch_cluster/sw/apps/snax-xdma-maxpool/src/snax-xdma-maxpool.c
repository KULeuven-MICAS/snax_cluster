...
int main(){
    ...
    if (snrt_is_dm_core()){
        ...
        snrt_dma_start_2d(tcdm_in, padded_data_in, padded_W * Cin * sizeof(uint8_t), 512 * sizeof(uint8_t), padded_W * Cin * sizeof(uint8_t), padded_H * sizeof(uint8_t));
        snrt_dma_wait_all();
        ...
        for (int i = 0; i < out_H; i++){
            for (int j = 0; j < out_W/8; j++){
                ...
                if (xdma_memcpy_nd(local_src_pointer, local_dst_pointer, 3, 1, stride_src, stride_dst, bound_src, bound_dst, 0xFF) != 0) {
        ...
        for (int i = 0; i < out_H*out_W*Cin; i++){
            if((int8_t)tcdm_out[i]!=golden_data_out[i]){
                ...
                printf("tcdm_out[%d]=%d, golden_data_out[%d]=%d",i,(int8_t)tcdm_out[i],i,golden_data_out[i]);
            }
        }
