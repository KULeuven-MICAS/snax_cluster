...
from snax_utils import max_pooling
np.random.seed(42)
...
def emit_data(**kwargs):
    ...
    data_str = ""
    data_str += format_scalar_definition("int8_t","H",kwargs["H"]) + "\n"
    ...
    out_h = (kwargs["H"] + 2 * kwargs["pad_h"] - kwargs["Kh"]) // kwargs["stride_h"] + 1
    out_w = (kwargs["W"] + 2 * kwargs["pad_w"] - kwargs["Kw"]) // kwargs["stride_w"] + 1
    ...
    data_str += format_vector_definition("int8_t", "padded_data_in", padded_data_in.reshape(-1)) + "\n"
