...
static inline uint32_t read_xdma_cfg_reg(uint32_t addr) {
    return csrr_ss(XDMA_CFG_ADDR + addr);
}
static inline void write_xdma_cfg_reg(uint32_t addr, uint32_t value) {
...
static inline uint32_t xdma_start() {
    int local_task_id = read_xdma_cfg_reg(XDMA_COMMIT_LOCAL_TASK_PTR);
    int remote_task_id = read_xdma_cfg_reg(XDMA_COMMIT_REMOTE_TASK_PTR);
    write_xdma_cfg_reg(XDMA_START_PTR, 1);
    while (1) {
        // Wait for xdma to start
        if (read_xdma_cfg_reg(XDMA_COMMIT_LOCAL_TASK_PTR) != local_task_id) {
            return read_xdma_cfg_reg(XDMA_COMMIT_LOCAL_TASK_PTR);
        }
        if (read_xdma_cfg_reg(XDMA_COMMIT_REMOTE_TASK_PTR) != remote_task_id) {
            return read_xdma_cfg_reg(XDMA_COMMIT_REMOTE_TASK_PTR);
        }
    }
}
...
static inline void xdma_local_wait(uint32_t task_id) {
    while (read_xdma_cfg_reg(XDMA_FINISH_LOCAL_TASK_PTR) < task_id) {
        // Wait for xdma to finish
    }
}

static inline void xdma_remote_wait(uint32_t task_id) {
    while (read_xdma_cfg_reg(XDMA_FINISH_REMOTE_TASK_PTR) < task_id) {
        // Wait for xdma to finish
    }
}

static inline uint32_t xdma_last_task_cycle() {
    return read_xdma_cfg_reg(XDMA_PERF_CTR_TASK);
}

static inline uint32_t xdma_last_read_cycle() {
    return read_xdma_cfg_reg(XDMA_PERF_CTR_READER);
}

static inline uint32_t xdma_last_write_cycle() {
    return read_xdma_cfg_reg(XDMA_PERF_CTR_WRITER);
}
