...
__attribute__((always_inline)) static inline void snax_xdma_dst_ext_csr_write(uint32_t slot, uint32_t val) {
    switch (slot) {  // XDMA_DST_EXT_CSR_NUM slots
        ...
        // Every slot needs its own arm: a slot with no arm falls through to default,
        // the extension keeps whatever CSR it already held, and the transfer runs with a
        // configuration nobody wrote. Keep this covering XDMA_DST_EXT_CSR_NUM slots.
