...
void set_gemmx_streamer_csr(int32_t* Aslstride, int32_t* Atlbound,
    ...
    for (int i = 0; i < S_STRIDE_NUM_WRITER_1; i++) {
        csrw_ss(S_STRIDE_BASE_WRITER_1 + i, D32slstride[i]);
    }
    ...
    if (bypassSIMD == 0) {
        ...
        for (int i = 0; i < T_BOUND_NUM_WRITER_1; i++) {
            csrw_ss(T_BOUND_BASE_WRITER_1 + i, D32tlbound[i]);
        }
    }

    // temporal strides for D32
    for (int i = 0; i < T_STRIDE_NUM_WRITER_1; i++) {
        csrw_ss(T_STRIDE_BASE_WRITER_1 + i, D32tlstride[i]);
    }

    // set the address remap index for D32
#ifdef ADDR_REMAP_INDEX_WRITER_1
    csrw_ss(ADDR_REMAP_INDEX_WRITER_1, set_addr_remap_index_D32);
#endif
