...
void set_streamer_csr(int tempLoop0, int tempLoop1, int tempLoop2,
    ...
    // base ptr for C
    write_csr(BASE_PTR_READER_WRITER_0_Low, (uint32_t)(delta_local_c + snrt_l1_next()));

    // spatial strides for C
    write_csr(S_STRIDE_READER_WRITER_0_0, spatialStride1C);

    // loop bounds, from innermost to outermost, from K to N to M
    write_csr(T_BOUND_READER_WRITER_0_0, tempLoop0);
    write_csr(T_BOUND_READER_WRITER_0_1, tempLoop1);
    write_csr(T_BOUND_READER_WRITER_0_2, tempLoop2);

    // temporal strides for C
    write_csr(T_STRIDE_READER_WRITER_0_0, 0);
    write_csr(T_STRIDE_READER_WRITER_0_1, tempStride1C);
    write_csr(T_STRIDE_READER_WRITER_0_2, tempStride2C);

    // base ptr for D
    write_csr(BASE_PTR_READER_WRITER_1_Low, (uint32_t)(delta_local_d + snrt_l1_next()));

    // spatial strides for D
    write_csr(S_STRIDE_READER_WRITER_1_0, spatialStride1C);

    // loop bounds, from innermost to outermost, from K to N to M
    write_csr(T_BOUND_READER_WRITER_1_0, tempLoop0);
    write_csr(T_BOUND_READER_WRITER_1_1, tempLoop1);
    write_csr(T_BOUND_READER_WRITER_1_2, tempLoop2);

    // temporal strides for D
    write_csr(T_STRIDE_READER_WRITER_1_0, 0);
    write_csr(T_STRIDE_READER_WRITER_1_1, tempStride1C);
    write_csr(T_STRIDE_READER_WRITER_1_2, tempStride2C);
