...
module snitch_cc #(
  ...
  /// AXI DMA AR/AW channel types (needed by idma_inst64_top).
  ...
  if (Xdma) begin : gen_dma
    // We only have one DMA channel
    axi_req_t    [0:0] dma_axi_req;
    axi_rsp_t    [0:0] dma_axi_res;
    logic        [0:0] dma_busy;
    dma_events_t [0:0] dma_ev;

    assign axi_dma_req_o    = dma_axi_req[0];
    assign dma_axi_res[0]   = axi_dma_res_i;
    assign axi_dma_busy_o   = dma_busy[0];
    assign axi_dma_events_o = dma_ev[0];
