      ...
      test(new ArrayTopHarness(params)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
        val rand = new Random()
            (0 until params.opType.length).map{ dataTypeIdx =>
              ...
              val expectedResult = Array.tabulate(M, N) {
                  (m2, n2) =>{
                      // multiply a and b and sum up along K dimension
                      val mul_res = (0 until K).map{ k2 =>{
                          Array.tabulate(Mu, Nu) { (m1, n1) =>
                          (0 until Ku).map { k1 =>
                              val aSInt = toSInt(aValues(m2 * K * Mu * Ku + k2 * Mu * Ku + k1 + m1 * Ku), inputAElemWidth, params.opType(dataTypeIdx) == OpType.SIntSIntOp) // Convert UInt to SInt
                              val bSInt = toSInt(bValues(n2 * K * Nu * Ku + k2 * Nu * Ku + k1 + n1 * Ku), inputBElemWidth, params.opType(dataTypeIdx) == OpType.SIntSIntOp) // Convert UInt to SInt
                              aSInt * bSInt
                          }.sum
                      } 
                      }
                  }.reduce { 
                        (a, b) => a.zip(b).map { case (rowA, rowB) =>
              ...
              concurrent_threads = concurrent_threads.fork {
                ...
                for (temporalIndexInput <- 0 until M * K * N) {
                  ...
                  // A
                  val aValues_cur = aValues.slice(indexA * Mu * Ku, indexA * Mu * Ku + Mu * Ku).zipWithIndex.map { case (v, i) => BigInt(v) << (i * inputAElemWidth) }.sum
              ...
              concurrent_threads = concurrent_threads.fork {
                ...
                for (temporalIndexInput <- 0 until M * K * N) {
                  val (indexA, indexB) = temporalToSpatialIndicesAB(temporalIndexInput, K = K, N = N)
                  // B
                  val bValues_cur = bValues.slice(indexB * Nu * Ku, indexB * Nu * Ku + Nu * Ku).zipWithIndex.map { case (v, i) => BigInt(v) << (i * inputBElemWidth) }.sum
                  dut.clock.step(Random.between(1, 5))
              ...
              concurrent_threads = concurrent_threads.fork {
                ...
                for (temporalIndex <- 0 until M * N) {
                  val cValues_cur = cValues.slice(temporalIndex * Mu * Nu, temporalIndex * Mu * Nu + Mu * Nu).zipWithIndex.map { case (v, i) => BigInt(v) << (i * inputCElemWidth) }.sum
              ...
              concurrent_threads = concurrent_threads.fork {
                for (outputTemporalIndex <- 0 until M * N) {
                  ...
                  // Check the output
                  val expected = expectedResult.flatten.flatten.flatten.slice(outputTemporalIndex * Mu * Nu, (outputTemporalIndex + 1) * Mu * Nu)
                  ...
                  assert(output(i) == expected(i), f"Mismatch at index $i: got 0x${output(i)}%X, expected 0x${expected(i)}%X")
                  }
                  //
                  dut.clock.step(Random.between(1, 5))
