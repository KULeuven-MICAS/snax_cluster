...
class FPMULInt8Test extends AnyFlatSpec with ChiselScalatestTester with fpUtils with FPMULIntTestUtils {
  ...
  it should "perform fp16-int8 multiply correctly" in {
    test(
      ...
      test_fp_mul_int(dut, 1, A, B, C, 8)

      A = 0.5f
      B = 2
      C = 2.3f

      test_fp_mul_int(dut, 2, A, B, C, 8)

      A = 0.005f
      B = 7
      C = 100f

      test_fp_mul_int(dut, 3, A, B, C, 8)

      A = 0.005f
      B = 0
      C = 10f

      test_fp_mul_int(dut, 4, A, B, C, 8)
