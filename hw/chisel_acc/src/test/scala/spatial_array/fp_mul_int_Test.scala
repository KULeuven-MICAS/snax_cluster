...
trait FPMULIntTestUtils extends fpUtils {
  ...
  def test_fp_mul_int(dut: FPMULInt, test_id: Int, A: Float, B: Int, C: Float) = {
...
class FPMULInt8Test extends AnyFlatSpec with ChiselScalatestTester with fpUtils with FPMULIntTestUtils {

  behavior of "FPMULInt8"

  it should "perform fp16-int8 multiply correctly" in {
    test(
      new FPMULInt(
      ...
      test_fp_mul_int(dut, 1, A, B, C)

      A = 0.5f
      B = 2
      C = 2.3f

      test_fp_mul_int(dut, 2, A, B, C)

      A = 0.005f
      B = 7
      C = 100f

      test_fp_mul_int(dut, 3, A, B, C)

      A = 0.005f
      B = 0
      C = 10f

      test_fp_mul_int(dut, 4, A, B, C)
  ...
  it should "perform fp16-int2 multiply correctly" in {
    test(
      ...
      val testCases = Seq.fill(test_num)(
        (
          (random.nextFloat() * 20 - 10), // A: Random float between -10 and 10
          random.nextInt(3),              // B: Random int between 0 and 15
          (random.nextFloat() * 20 - 10)
        )                                 // C: Random float between -10 and 10
      )
