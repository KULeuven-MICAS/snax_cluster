...
/** FpExp: per-lane exp(x), FP32 in / FP32 out.
  *
  * NOTE: superseded in the StreamMap datapath by the merged `FpActivation` (which shares its FP units with silu). FpExp
  * is kept as the standalone GOLDEN REFERENCE — FpActivationTester diff-checks FpActivation's exp mode against it
  * bit-for-bit. It is no longer instantiated in the generated netlist.
  *
  * LUT-based 2^m design (the chosen area point): exp(x) = 2^m with m = x*log2e. Split m*lutN into an integer iM =
  * round(m*lutN); then n = iM >> log2(lutN) is the integer part of m and idx = iM[log2(lutN)-1:0] indexes a
  * `lutN`-entry ROM of 2^(idx/lutN); the leftover frac = m*lutN - iM in [-0.5,0.5] is folded with a single
  * interpolation FMA (2^(frac/lutN) ~= 1 + frac*ln2/lutN). Finally scale by 2^n (exponent-field construct). The
  ...
  *
  * PIPELINING (timing): the math is a ~7-deep FP32 chain (clamp->mul->add->add->add->fma->mul->mul), which over-runs
  * the 2 ns clock as one combinational path. When `pipelined` is set the chain is cut into `latency` (=4) register
  * stages of <=2 FP ops each; the stages are pure feed-forward so the result simply appears `latency` cycles later (the
  * host FSM accounts for it). `pipelined=false` keeps the original 1-cycle combinational behaviour (used by the
  * standalone tester).
  *
  * AREA: `lutN` (ROM depth) trades table size for interpolation accuracy. 128 entries stay within the relaxed <=2 FP16
  * ULP budget on the softmax range while halving the ROM vs the original 256.
  */
...
object FpExp {
  ...
  /** Register-stage latency of the pipelined datapath (kept in sync with the `sr()` cuts above and with
    * [[FpSilu.PipeLatency]] so StreamMap's func-mux branches line up).
    */
