...
/** The parent (abstract) Class for the DMA Extension Generation Params This class template is used to isolate the
  * definition of class (when user provide the parameters) and the actual instantiation of Module, and allow other
  * module to read some information about the Extension before the instantiation The idea is similar to Rocketchip's
  * LazyModule and LazyModuleImp.
  *
  * Usage:
  *
  * 1) For every custom extension "CustomExtension", an object "HasCustomExtension" should be declared, extending from
  * HasDataPathExtension.
  *
  * 2) @extensionParam, or basic parameters consumed by DataPathExtension parent class needs to be provided.
  *
...
/** The parent (abstract) Class for the DMA Extension Implementation (Circuit) All classes need to extends from this
  * parent class like below: class CustomModule(userParams)(implicit extensionParam: DataPathExtensionParam) extends
  * DataPathExtension Inside the body of CustomModule, the following thing must be done:
  *
  * 1) Connect ext_data_i to your module's datapath input: ext_data_i <> userDefinedInput.
  *
  * 2) Connect ext_data_o to your module's datapath output: ext_data_o <> userDefinedOutput.
  *
  * 3) Connect CSR to your module <> userDefinedCSR := ext_csr_i.
  *
  * 4) Connect Start signal: userDefinedStart := ext_start_i. The start signal is used to inform extension a new stream
  * is coming.
  *
...
/** The parent Class for the integration of DMA Extension written in SystemVerilog Before the integration of the custom
  * extension, please make sure the following:
  *
  ...
  *
  * 1> Two parameters is provided by XDMA generator: userCsrNum and dataWidth, which is user-definable in
  * HasDataPathExtension class.
  *
  ...
  *
  * 4> csr_i is the CSR signal, and it is an unrolled two-dimensional signal. If userCsrNum is 2, then there is two
  * csrs: ext_csr_i_0 and ext_csr_i_1.
  *
  * 2) After the writing of the SystemVerilog module, give Chisel generator the method to integrate the SystemVerilog
  * module. This is done by the declaration of HasDataPathExtension object. (Take a llok at example in
  * VerilogMemset.scala)
  *
  * 3) The instantiate method should be provided, with a new SystemVerilogDataPathExtension class.This class need two
  * parameters: topmodule and filelist. The topmodule is the name of the SystemVerilog module (very similar to defining
  * the top module in the backend flow), and the filelist is the list of SystemVerilog files that are needed to be
  * integrated. 4) The location of SystemVerilog file doesn't matter. Because Chisel will include all the code you write
  * inside the body of XDMA. However, it is recommended to put it in the src/main/systemverilog folder for the
  * management purpose.
  */
class SystemVerilogDataPathExtension(topmodule: String, filelist: Seq[String])(implicit
