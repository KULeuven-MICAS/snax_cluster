...
/** StreamMap: per-lane pointwise map act(a*x + b) over a streamed row.
  *
  *   - x is a transport-precision lane (the element type — FP16/BF16/FP8/FP32 — is set by the op-set); a, b are FP32
  *     CSR-immediates; the math is FP32; the output is narrowed back to the transport type.
  *   - func in {LINEAR, EXP, SILU} selected at runtime (CSR), so one instance serves softmax pass-2 (a=1, b=-max, EXP),
  *     pass-4 (a=inv_sum, b=0, LINEAR — identity activation, just a*x+b), and silu (a=1, b=0, SILU). EXP and SILU are
  *     computed by ONE merged FpActivation core (they are mutually exclusive at runtime; a func bit selects exp vs
  *     silu, sharing all its FP units — see FpActivation).
  *
  * TIME-MULTIPLEXED (area-reduction point): only `computeLanes` compute units are built and reused over `subCycles` (=
  * lanes/computeLanes) cycles per beat, so a 512-bit beat (e.g. 32 FP16 lanes) needs only `computeLanes` activation
  * cores instead of one per lane. (computeLanes >= lanes ⇒ fully parallel, one sub-cycle.) The issue FSM STREAMS: it
  * feeds one sub-group per cycle across back-to-back beats and reassembles the results into whole output beats through
  * a small skid FIFO, so the per-lane pipeline latency P is a one-time fill rather than a per-beat bubble. Steady-state
  * throughput = 1 beat / `subCycles` cycles (1 beat/cycle at computeLanes == lanes), independent of P.
  *
  * CSR layout: csr(0)=a (FP32 bits), csr(1)=b (FP32 bits), csr(2)= bits[1:0]=func (0=LINEAR,1=EXP,2=SILU). a, b are
  * plain FP32 immediates; the normalize maps (softmax x/Σ, rmsnorm x·rms) pass the already-inverted scalar in `a` as a
  * LINEAR multiply — the host computes the 1/Σ resp. 1/√mean reciprocal/rsqrt.
  *
  * Configurable from the hjson: `computeLanes` (time-mux width), `func` (the LIST of supported funcs, a subset of
  * {LINEAR, EXP, SILU}), and `elementWidth` (the transport element width in bits — must match the func-set precision:
  * FP16/BF16⇒16, FP8⇒8, FP32⇒32). The FpActivation's exp / silu tables+path are built only if "EXP"/"SILU" is listed;
  * the act CSR field selects among the listed funcs at runtime when >1 listed. All are REQUIRED (no defaults) — the
  * config must spell them out explicitly, e.g. {elementWidth:16, computeLanes:8,
  * func:["LINEAR_FP16","EXP_FP16","SILU_FP16"]}.
  *
  * HARDWARE UNIT COUNT — for a 512-bit input only `computeLanes` physical compute units exist, NOT one per lane (lanes
  * \= dataWidth/elementWidth, e.g. 32 at FP16 or 64 at FP8): each does one mixed FFMA (a·x+b) and, when EXP/SILU is
  * listed, one merged FpActivation. They are time-multiplexed over `subCycles` (= lanes/computeLanes) cycles to cover
  * all lanes, so a func-set builds `computeLanes` cores. computeLanes = lanes ⇒ subCycles=1 ⇒ one unit per lane in
  * parallel (the combinational path).
  *
  * NUMERICAL EXAMPLE — affine map, FP16 transport, 32 lanes, computeLanes=8 ⇒ subCycles=4. CSR a=2.0, b=1.0,
  * func=LINEAR ⇒ out[i]=2·x[i]+1 (same op every lane). The 8 physical compute units U0..U7 are REUSED across the 4
  * sub-cycles; on sub-cycle s, unit Uj processes logical lane (s·8+j), so one 32-lane beat takes 4 compute cycles (+1
  * to emit):
  *
  * CC phase U0 U1 U2 U3 U4 U5 U6 U7 → outBeat lanes written -- ------- ------------------------------------
  * ---------------------- 0 accept latch the 512-bit input x[0..31] — 1 sub=0 x0 x1 x2 x3 x4 x5 x6 x7 [0..7] 2 sub=1 x8
  * x9 x10 x11 x12 x13 x14 x15 [8..15] 3 sub=2 x16 x17 x18 x19 x20 x21 x22 x23 [16..23] 4 sub=3 x24 x25 x26 x27 x28 x29
  * x30 x31 [24..31] (last sub) 5 emit drive ext_data_o = [2·x0+1, …, 2·x31+1] e.g. x0=3, x1=5 ⇒ out0=7, out1=11. Lane
  * reuse: unit Uj owns logical lanes {j, 8+j, 16+j, 24+j}. For func=EXP/SILU each Uj additionally holds an FpActivation
  * (8 instead of 32). computeLanes=32 ⇒ subCycles=1 ⇒ the combinational path: all 32 lanes in one cycle, no
  * sub-cycling.
  */
class HasStreamMap(
  computeLanes: Int,
