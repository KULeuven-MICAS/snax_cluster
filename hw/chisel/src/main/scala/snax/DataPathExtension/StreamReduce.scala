...
class HasStreamReduce(
  ...
  private val (_, transport) = OpSpec.parse(op, Set("MAX", "ADD", "SUMSQ", "FMA"), "HasStreamReduce") // validate op names + precision
...
class StreamReduce(
  ...
  def fmaP(x: UInt, y: UInt, c: UInt): UInt = {
    val m = Module(new FpFma(transport, transport, FP32, fmaPipe)); m.io.in_a := x; m.io.in_b := y; m.io.in_c := c; m.io.out
  }
  ...
  when(ext_start_i) {
    ...
    when(!treeBusy) {
      ...
      when(foldWriteback) {
        ...
        foldWait := 0.U
        when(lastChunk) {
          when(foldRound === (numLevels - 1).U) { treeBusy := false.B; treeDone := true.B }
            .otherwise { foldRound := foldRound + 1.U; foldCol := 0.U }
