...
/** FpSilu: per-lane silu(x) = x * sigmoid(x), FP32 in / FP32 out.
  *
  * sigmoid is read from an `N`-entry FP32 ROM using ODD SYMMETRY to halve the table: we tabulate only g(m) =
  * sigmoid(-m) for m = |x| in [0, 16] (step h = 16/N) — the small, fully-accurate side — and recover sigmoid(x) =
  * (x<=0) ? g(|x|) : 1 - g(|x|) (sigmoid(-m)=1-sigmoid(m)). A single central- difference interpolation FMA refines g
  * (sigmoid' is even, so one slope table serves both halves); the result is then multiplied by the ORIGINAL signed x to
  * form silu. Tabulating g(|x|) (not raw sigmoid over [-16,16]) keeps the deep-negative tail — where silu ~ x*e^x lands
  * small normals on a fine FP16 grid — at full FP32 precision: it is read DIRECTLY from the ROM, never via a cancelling
  * subtraction. The only subtraction (1 - g) is on the x>0 side where g is the tiny operand, so it is benign. The
  * x-multiply makes the tails exact (x>=16: sigmoid~1 => silu~x; x<=-16: sigmoid~0 => silu~0). The float->index
  * conversion uses the same 1.5*2^23 magic-number round as FpExp (no F2I unit).
  *
  * PIPELINING (timing): like FpExp, the ~7-deep FP32 chain is cut into `latency` (=4) register stages of <=2 FP ops
  * each when `pipelined` is set (the original x is carried down the pipe for the final multiply); the host FSM accounts
  * for the latency. `pipelined=false` keeps the original combinational behaviour.
  *
  * AREA: `N` (ROM depth, ×2 for base+slope) is the node count over [0,16]. Odd symmetry gives the same node density as
  * the old 512-over-[-16,16] table at N=256, halving both ROMs while staying within <=1 FP16 ULP (the prior uniform
  * 256-node [-16,16] table measured 3 ULP at x~-10.8; symmetry fixes that).
  *
  * Same shape and role as FpExp (one ROM family + one interp FMA). NOTE: superseded in the StreamMap datapath by the
  * merged `FpActivation` (which computes exp OR silu on shared FP units). FpSilu is kept as the standalone GOLDEN
  * REFERENCE that FpActivationTester diff-checks silu mode against bit-for-bit; it is no longer instantiated in the
  * generated netlist.
  */
class FpSilu(pipelined: Boolean = false, N: Int = 256) extends Module with RequireAsyncReset {
  ...
  val slope = VecInit((0 until N).map(i => f32lit(((g(nodeX(i + 1)) - g(nodeX(i - 1))) / 2.0).toFloat)))
...
object FpSilu {
  ...
  /** Register-stage latency of the pipelined datapath (kept in sync with the `sr()` cuts above and with
    * [[FpExp.PipeLatency]] so StreamMap's func-mux branches line up).
    */
