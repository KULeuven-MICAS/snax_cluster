...
/** FpActivation: one per-lane activation core that computes exp(x) OR silu(x) (FP32 in/out), selected at
  * runtime by `io.func` (false = exp, true = silu). It MERGES the former standalone FpExp and FpSilu, which
  * were both instantiated per StreamMap lane yet are mutually exclusive at runtime. The two share the exact
  * same 4-stage skeleton — input-affine -> magic-round -> fractional split -> LUT + interpolation FMA ->
  * post-transform — so all seven FP units are shared (operands muxed by `func`); only the ROMs stay
  * per-function (exp: one 2^(i/lutN) table; silu: base+slope sigmoid tables). This halves the activation FP
  * area of StreamMap versus the two separate modules, bit-exactly (see the fmul==ffma(_,_,+0) and
  * 1-g==ffma(g,-1,+1) identities below), while keeping the same PipeLatency=4 the func-mux relies on.
  *
...
class FpActivation(
  ...
  val lut      = if (hasExp) VecInit((0 until expLutN).map(i => f32lit(math.pow(2.0, i.toDouble / expLutN.toDouble).toFloat)))
  ...
  val slope  = if (hasSilu) VecInit((0 until siluN).map(i => f32lit(((gnode(i + 1) - gnode(i - 1)) / 2.0).toFloat)))
               else VecInit(Seq(ZERO))
...
object FpActivation {
  /** Register-stage latency of the pipelined datapath (matches the former FpExp/FpSilu PipeLatency so
    * StreamMap's LINEAR/EXP/SILU func-mux branches stay aligned). */
