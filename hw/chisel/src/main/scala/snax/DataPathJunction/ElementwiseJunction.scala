...
/** \============================================================================================================
  ...
  * \============================================================================================================
  *
  * A 2-input, per-element reduction (ADD / MUL / MAX / MIN) on the Junction ABI: same arity, same chain position and
  * same per-beat throughput as the monoid operator, but per element and with no coupling between lanes.
  *
  * This is one of the two operators shipped on the junction socket. It satisfies the same four obligations
  * `MonoidJunction` does -- declared latency, format closure, identity tolerance, no state between pairs -- with an
  * entirely different internal shape, which is the point: the socket constrains behaviour, not structure.
  *
  * ADD / MUL / MAX / MIN commute with routing, so a chain of these nodes computes the same result regardless of how the
  * reduction is ordered along the route. The nonlinear family in `MonoidCombine` -- the online-softmax (m, l) merge,
  * the flash-attention (m, l, O) triple, the exp-weighted moment bank -- is NOT expressible this way at any element
  * granularity, because it needs a shared rescale alpha = exp(m_loser - m*) derived from the operand pair itself.
  *
  * ---- TWO GRIDS ----
  *
  * FLOATING POINT. `elemWidth` (elaboration) sets the lane count `lanes = dataWidth / elemWidth`; the math is always
  * FP32, so `fmt` (runtime) selects any transport format at least `elemWidth` wide and the beat is sliced accordingly.
  * Every supported format runs at ONE BEAT PER CYCLE -- narrower formats simply light up more lanes. At the default
  * `elemWidth = 16` that is 32 FP32 lanes covering FP16/BF16 at full beat rate, and FP32 on lanes 0..15.
  *
  * INTEGER. `intWidths` (elaboration) selects which of INT8 / INT16 / INT32 are built. Integers are combined at their
  * NATIVE width -- no widen, no narrow, no rounding -- so an integer format is exact and the beat is `dataWidth / w`
  * lanes wide whatever `elemWidth` is. Two properties follow, and both are the point of having this grid at all:
  *
  *   - `ADD` and `MUL` WRAP, i.e. they are the group and ring operations of Z_2^w. Wrapping addition is EXACTLY
  *     associative and exactly commutative, so a chain of these nodes returns one answer whatever the route -- which
  *     floating-point addition, on the grid above, does not.
  *   - `MAX` / `MIN` are SIGNED comparisons.
  *
  * That makes the integer grid the merge rule of the standard mergeable summaries: a HyperLogLog merges by per-register
  * `MAX` over INT8, a Count-Min sketch by elementwise `ADD` over INT32, a G-Counter by per-replica `MAX`, and a
  * tropical `(min, +)` distance vector by elementwise `MIN`.
  *
  * ---- CSR(0) ----
  * {{{
...
class HasElementwiseJunction(
  ...
  def instantiate(clusterName: String): ElementwiseJunction =
    Module(
      new ElementwiseJunction(elemWidth = elemWidth, fpPipe = fpPipe, intWidths = intWidths, skidDepth = skidDepth) {
...
class ElementwiseJunction(
  ...
  require(
    intWidths.forall(w => Seq(8, 16, 32).contains(w)),
