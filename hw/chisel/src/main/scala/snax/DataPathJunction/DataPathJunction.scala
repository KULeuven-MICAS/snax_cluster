...
/** \============================================================================================================
  ...
  *   example             StreamMap, StreamReduce             MonoidJunction, ElementwiseJunction
  * }}}
  *
  ...
  *      forward, and `b_i` is not consumed, so a disabled junction is functionally inert. 2. STARVATION VISIBILITY. A
  *      two-stream join stalls indefinitely if the two operand streams do not carry the same number of beats, and
  *      beat-count equality is a software contract on two independently dispatched cfgs with no hardware backstop.
  *      Every junction therefore exposes `starved_o`, raised when one side has waited on the other for longer than
  *      `starveLimit` cycles.
  *
  * IMPLEMENTATION CONTRACT for a concrete junction:
  *   1. drive `jct_a_i` / `jct_b_i` from your datapath inputs, 2. drive `jct_data_o` from your datapath output, 3. read
  *      config from `jct_csr_i`, the new-stream pulse from `jct_start_i`, 4. drive `jct_busy_o` low only when nothing
  *      is in flight.
  */
class JunctionParam(
...
abstract class DataPathJunction(implicit junctionParam: JunctionParam) extends Module with RequireAsyncReset {
  ...
  /** O5 -- DECLARED SCHEMA. A junction's configuration arrives over a serdes from another die and nothing on the path
    * checks it, so a word that this operator cannot honour would otherwise produce a finite, format-legal, silently
    * wrong beat -- the species of bug that has already been found twice in this block. Raise this instead, from the CSR
    * alone, so it is stream-constant and asserted before the first pair fires.
    *
    * It reports; it does not refuse. The datapath still emits its safest interpretation, because a fold that stops
    * mid-chain is worse than one that completes and is flagged.
    *
  ...
  /** O3, STRENGTHENED. Tolerating an identity is not the same as being able to PRODUCE one, and the difference matters
    * the moment a shard dies or a link drops: the chassis needs a beat it can put on the missing operand, and only the
    * operator knows what that beat is. It is not a constant and not even a per-operator constant -- it is a function of
    * the configuration word (a max-monoid's key identity is the most-negative float, a min-monoid's the most-positive,
    * an ordered scan's is 1.0, and `AND`'s is all ones where `OR`'s is all zeros). Asking the chassis to guess it is
    * how a fold gets silently annihilated.
    *
...
/** \============================================================================================================
  ...
  * need a third stream. The host routes the operand pair to the one junction whose enable bit is set, and falls back to
  * a transparent a -> out forward when none is.
  *
...
class DataPathJunctionHostIO(junctionList: Seq[HasDataPathJunction], dataWidth: Int = 512) extends Bundle {
  ...
  val cfg  = new Bundle {
    ...
    /** `[n-1:0]` one enable bit per operator. `[16]` and `[17]` ask the socket to SUBSTITUTE the armed operator's
      * identity for operand A or operand B, so a dead shard or a dropped link is a configuration rather than a hang.
      * They ride the spare bits of the enable word, so no new CSR and no change to the address map.
      */
...
class DataPathJunctionHost(
  ...
  if (junctionList.isEmpty) {
    ...
    assert(PopCount(VecInit(enables)) <= 1.U, "DataPathJunctionHost: at most one junction may be enabled per transfer")
    ...
    def selBits(f: DataPathJunction => UInt): UInt = Mux1H(enables, junctions.map(f))
