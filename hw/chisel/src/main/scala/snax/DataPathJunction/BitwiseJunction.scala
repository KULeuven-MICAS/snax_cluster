...
/** ============================================================================================================
  ...
  * }}}
  * All three are commutative monoids, so all three are legal collectives. `OR` is the union of Bloom filters and
  * of presence bitmaps, `AND` their intersection, and `XOR` is parity -- the reduction an erasure code or an
  * additively-shared secret is aggregated with.
  *
  * WHY IT IS HERE. Every other operator on this socket was designed around a datapath: the twisted family around
  * an exponential and a fused multiply-add, the linear operator around a lane grid, top-k around a comparator
  * network. This one has none of those. It contains no adder, no multiplier, no comparator and no state beyond a
  * single retiming stage, and it still satisfies the same four obligations. If the contract were secretly a
  * description of the monoid cell, this operator could not exist.
  *
  ...
  * }}}
  *
  * `XOR` is worth one line of care. It is a legal collective -- associative, commutative, with identity 0 -- but
  * it is NOT idempotent, so unlike `OR` and `AND` a duplicated beat corrupts it. An operator declaring
  * duplicate-tolerance would have to declare it per op, not per operator.
  *
  * ---- CSR(0) ----
  * {{{
...
class BitwiseJunction(
  ...
  val outBeat = RegNext(MuxLookup(opcode, a | b)(
