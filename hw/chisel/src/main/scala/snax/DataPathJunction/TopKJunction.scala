...
/** \============================================================================================================
  ...
  * }}}
  * The two are the same network under negation of every value, so the polarity is one XOR on each comparator and a
  * different identity constant -- no second sorting network, and the latency is identical.
  *
  * The `keyPol = 1` reading is a BOTTOM-k / KMV sketch merge: keeping the k smallest hash values of a union is how
  * MinHash estimates Jaccard similarity and how a Theta sketch estimates cardinality. Both readings are the same beat
  * layout and the same 20 comparators. Associative and commutative (both sides are "the k largest of the union"),
  * bounded (k values + k payloads, fixed), and format-closed by construction -- a sorted k-tuple in, a sorted k-tuple
  * out, same layout.
  *
  * WHY THIS OPERATOR. It meets the classified family at exactly one point: `k = 1` with a payload IS argmax, the
  * selection corner. For `k > 1` there is no single key and no scalar rescale, so it is **outside the family entirely**
  * -- which makes it the evidence that the socket's contract, not the monoid cell, is what composes. It also uses a
  * beat layout that is NOT `lane = field*S + slot`, and does not have to: the contract requires closure, not a
  * geometry.
  *
  ...
  * }}}
  * Stage 0 is k selects; each later stage is k/2 compare-exchanges. At k = 8 that is 8 + 12 = 20 comparators in 4
  * registered stages -- NOT the fully combinational 16-input Batcher network that is the hazard on record. There is no
  * key front end, no exponential unit and no FMA anywhere in this operator.
  *
  * ---- CSR(0) ----
  * {{{
  ...
  * }}}
  * `keyPol` sits at bit 28 because that is where `MonoidJunction` carries the same meaning, so one software macro sets
  * the monoid polarity for either operator.
  *
  * TIES: the comparison is `>=` (or `<=` at `keyPol = 1`), so an exact tie keeps the A-side entry -- the same
  * deterministic rule `ARGMAX` uses. With distinct values the operator is exactly commutative; on tied values the VALUE
  * is still commutative and only the carried payload can differ.
  */
...
class TopKJunction(
  ...
  /** the value that loses every comparison, so it is the identity of whichever end is being kept: the most-negative
    * finite FP32 for a max, the most-positive for a min. FINITE on purpose -- a true infinity would make an
    * identity-against-identity comparison meaningless, exactly as it does for the monoid operator.
    */
