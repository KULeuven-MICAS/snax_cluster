...
/** \============================================================================================================
  ...
  * }}}
  *
  * A multicast chain hands each hop a COPY (the splitter). A gather chain hands each hop a FOLD: the arriving partial
  * meets this node's local operand at the junction and only the combined result travels on. The two are protocol
  * inverses -- same chain unroll, same Grant/Finish direction, one different block at the crossing.
  *
  * ---- MODE DERIVATION ----
  *
  * A gather is a chained transfer whose JUNCTION is enabled, and the junction's enable bitmask rides the writer-side
  * `extCfg` region, which crosses the inter-cluster serdes to every hop. So:
  * {{{
  ...
  * `writerCfg.remoteLoopback && writer.io.busy`. A gather MIDDLE node does not write locally, so `writer.io.busy` never
  * asserts: `readyToTransfer` would never assert, the grant manager would never leave `IDLE`, and the chain would stall
