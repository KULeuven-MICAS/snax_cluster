...
class XDMADataPathParam(
  ...
  def extCsrNum: Int =
    if (extParam.isEmpty) 0 else extParam.map(_.extensionParam.userCsrNum).sum + 1

  /** CSRs occupied by the junction bank, laid out identically and placed AFTER the extension region (0 if none). */
  def junctionCsrNum: Int =
    if (junctionParam.isEmpty) 0 else junctionParam.map(_.junctionParam.userCsrNum).sum + 1
