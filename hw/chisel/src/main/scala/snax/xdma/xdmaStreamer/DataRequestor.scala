...
class DataRequestorsIO(tcdmDataWidth: Int, tcdmAddressWidth: Int, isReader: Boolean, numChannel: Int) extends Bundle {
  val in = new Bundle {
    val addr = Vec(numChannel, Flipped(Decoupled(UInt(tcdmAddressWidth.W))))
    val ResponsorReady = if (isReader) Some(Vec(numChannel, Input(Bool()))) else None
    val data = if (!isReader) Some(Vec(numChannel, Flipped(Decoupled(UInt(tcdmDataWidth.W))))) else None
  }
...
/** DataRequestor's IO definition:
  * io.in.address: Decoupled(UInt)
  * io.in.data: Some(Decoupled(UInt)) or None
  * io.in.ResponsorReady: Some(Bool()) or None
  * io.out: Toward TCDM, see Xiaoling's definition to compatible with her wrapper: TypeDefine.scala
  */
class DataRequestor(tcdmDataWidth: Int, tcdmAddressWidth: Int, isReader: Boolean)
    extends Module
  ...
  io.in.addr.ready := { if (isReader) io.in.ResponsorReady.get && io.out.tcdm_req.fire else io.out.tcdm_req.fire }
  ...
  io.out.tcdm_req.valid := { if (isReader) (io.in.addr.valid && io.in.ResponsorReady.get) else (io.in.addr.valid && io.in.data.get.valid) }

}

/** DataRequestors' IO definition:
  * io.in.address: Vec(Decoupled(UInt))
  * io.in.data: Some(Vec(Decoupled(UInt))) or None
  * io.in.ResponsorReady: Some(Vec(Bool())) or None
  * io.out: Toward TCDM, see Xiaoling's definition to compatible with her wrapper
  */
...
class DataRequestors(tcdmDataWidth: Int, tcdmAddressWidth: Int, isReader: Boolean, numChannel: Int)
    extends Module
    with RequireAsyncReset {
  val io = IO(new DataRequestorsIO(tcdmDataWidth, tcdmAddressWidth, isReader, numChannel))
  val DataRequestor = for (i <- 0 until numChannel) yield {
    val module = Module(new DataRequestor(tcdmDataWidth, tcdmAddressWidth, isReader))
...
object DataRequestorTester extends App {
  emitVerilog(new DataRequestor(64, 16, true), Array("--target-dir", "generated"))
}

object DataRequestorsTester extends App {
  emitVerilog(new DataRequestors(64, 16, false, 2), Array("--target-dir", "generated"))
}
