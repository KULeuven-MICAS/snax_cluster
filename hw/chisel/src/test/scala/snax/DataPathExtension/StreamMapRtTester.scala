...
class StreamMapRtTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  def packF4(v: Seq[Int]): BigInt = v.zipWithIndex.foldLeft(BigInt(0)) { case (a, (x, i)) => a | (BigInt(x & 0xf) << (4 * i)) }

  "StreamMapRt_mxfp4" should "process 128 MXFP4 elems/beat (4x ladder) at the area-optimal cl=64 (util~0.5 = 2x compute)" in {
    // cl=64 is the recommended area point: MXFP4 = 128 elems over runSub=2 cycles = 64 elem/cyc = 2x FP16
    // compute, with 4x fewer BEATS/element (the memory win). cl=128 (full 4x compute) is opt-in but too large
    // to build under Verilator here.
    test(new DataPathExtensionHarness(new HasStreamMapRt(computeLanes = 64, func = Seq("LINEAR"))))
      .withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
        val rng = new Random(0x4f)
        // E2M1 normal codes: exp in {1,2,3} (bias 1), mant in {0,1}
        def normF4(): Int = (if (rng.nextBoolean()) 8 else 0) | ((1 + rng.nextInt(3)) << 1) | rng.nextInt(2)
        val csr2mx = BigInt(0) | (BigInt(7) << 2) | (BigInt(127) << 5) // LINEAR | fmt=MXFP4(7)<<2 | scale=127<<5
        // --- correctness: identity map (a=1,b=0) round-trips each E2M1 code (scale-neutral) ---
        val elems = Seq.fill(128)(normF4())
        ...
        th = th.fork { dut.io.data_i.bits.poke(packF4(elems).U); dut.io.data_i.valid.poke(true)
          while (!dut.io.data_i.ready.peekBoolean()) dut.clock.step(1); dut.clock.step(1); dut.io.data_i.valid.poke(false) }
        th = th.fork { while (!dut.io.data_o.valid.peekBoolean()) dut.clock.step(1)
          o = dut.io.data_o.bits.peekInt(); dut.io.data_o.ready.poke(true); dut.clock.step(1); dut.io.data_o.ready.poke(false) }
        th.joinAndStep()
        var mism = 0
        for (i <- 0 until 128) { val got = ((o >> (4 * i)) & 0xf).toInt; if (got != elems(i)) mism += 1 }
        assert(mism == 0, s"MXFP4 identity: $mism/128 codes mismatched")
        // --- roofline: MXFP4 at cl=128 = 128 elems/cycle = the full 4x (util=1.0) ---
        dut.io.data_o.ready.poke(true)
        val nIn = 200; val beats = Seq.fill(nIn)(packF4(Seq.fill(128)(normF4())))
        dut.io.start_i.poke(true); dut.clock.step(1); dut.io.start_i.poke(false)
        dut.io.data_i.valid.poke(true); dut.io.data_i.bits.poke(beats(0).U)
        var cyc = 0L; var fed = 0; var got = 0; var warm = -1L; var last = -1L
        while (got < nIn && cyc < nIn * 40 + 400) {
        ...
        println(f"[StreamMapRt MXFP4] 128 elems/beat identity OK; roofline cl=64 util=$util%.3f (= 64 elem/cyc = 2x FP16; 4x fewer beats/elem)")
        assert(util > 0.4 && util < 0.65, s"MXFP4 at cl=64 should be ~0.5 (128 elems / 2 subcycles), got $util")
      }
  }
