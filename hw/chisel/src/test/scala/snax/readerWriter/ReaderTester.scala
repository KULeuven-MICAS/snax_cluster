...
class ReaderTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  // Generate expected output data based on AGU configuration
  // Each output aggregates data from all enabled channels (spatialBounds)
  def generateExpectedOutputs(
    basePtr: BigInt,
    spatialStride: Int,
    spatialBound: Int,
    ...
    val outputs = mutable.ArrayBuffer[BigInt]()
    
    // Helper to iterate through all temporal loop combinations
    // Start from the last temporal dimension (innermost loop) and work backwards
    def iterateTemporalLoops(
      loopIndices: Array[Int],
      depth: Int,
      currentOffset: BigInt
    ): Unit = {
      if (depth < 0) {
        // Base case: compute the aggregated output for this iteration
        var aggregatedData: BigInt = 0
        for (spatialIdx <- 0 until spatialBound) {
          val addr = basePtr + currentOffset + (spatialIdx * spatialStride)
          val data = addr & 0xFFFFFFFFFFFFFFFFl // Same pattern as getTcdmData
          // Aggregate data: pack all channel data into one wide output
          aggregatedData |= (data << (spatialIdx * 64))
        }
        outputs += aggregatedData
      } else {
        // Recursive case: iterate through current temporal loop
        for (i <- 0 until temporalBounds(depth)) {
          loopIndices(depth) = i
          val offset = currentOffset + (i * temporalStrides(depth))
          iterateTemporalLoops(loopIndices, depth - 1, offset)
        }
      }
    }

    iterateTemporalLoops(Array.fill(temporalBounds.length)(0), temporalBounds.length - 1, 0)
    outputs.toSeq
  }
  ...
  "Reader: basic sequential read test" should "pass" in test(
    ...
    .withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation)) { dut =>
      ...
      val expectedOutputs = generateExpectedOutputs(
        basePtr = 0x1000,
        spatialStride = 8,
        spatialBound = 8,
        temporalStrides = Seq(64, 0, 0, 0, 0, 0),
        temporalBounds = Seq(35, 9, 1, 1, 1, 1)
      )
      ...
      println(s"  Expected outputs: ${expectedOutputs.length}")

      // Verify output data correctness
      println(s"\nValidating output data...")
      assert(receivedOutputs.length == expectedOutputs.length, 
        s"Output count mismatch: expected ${expectedOutputs.length}, got ${receivedOutputs.length}")
      
      for (i <- receivedOutputs.indices) {
        if (receivedOutputs(i) != expectedOutputs(i)) {
          println(f"Output $i mismatch:")
          println(f"  Expected: 0x${expectedOutputs(i)}%0128x")
          println(f"  Received: 0x${receivedOutputs(i)}%0128x")
          assert(false, s"Output data mismatch at index $i")
        }
      }
      
      println(s"All ${receivedOutputs.length} outputs validated successfully!")
    }
