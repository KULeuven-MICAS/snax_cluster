...
//       // 1. Set control signals (ready signals)
//       // 2. Clock step
//       // 3. Observe DUT outputs (valid signals)
...
//         // PHASE 2: Clock step - DUT now sees our control signals
//         dut.clock.step()
//         cycleCount += 1
        
//         // PHASE 3: Observe DUT outputs AFTER the clock edge
//         // Check for new TCDM requests that were registered on the posedge
//         for (channelIdx <- 0 until 8) {
//           tcdmPortStates(channelIdx) match {
//             case None =>
//               // Check if a handshake occurred (we had ready=true, check if valid was also true)
//               if (dut.io.tcdmReq(channelIdx).valid.peek().litToBoolean) {
...
//                 // Accept request and set random latency (0-3 cycles)
//                 val latency = random.nextInt(4)
//                 tcdmPortStates(channelIdx) = Some(TcdmPortState(addr, latency))
//                 println(f"Cycle $cycleCount%3d: Channel $channelIdx - Request accepted, latency=$latency cycles")
//               }
...
//       assert(dataReceived == 4, s"Expected exactly 4 data outputs, got $dataReceived")
//     }
...
//         // PHASE 3: Observe DUT outputs and update state
//         for (channelIdx <- 0 until 8) {
//           tcdmPortStates(channelIdx) match {
//             case None =>
//               // Check if a handshake occurred
//               if (dut.io.tcdmReq(channelIdx).valid.peek().litToBoolean) {
//                 val addr = dut.io.tcdmReq(channelIdx).bits.addr.peek().litValue
//                 val write = dut.io.tcdmReq(channelIdx).bits.write.peek().litToBoolean
//                 val strb = dut.io.tcdmReq(channelIdx).bits.strb.peek().litValue
