...
/** Tier-1 for `TopKJunction` -- the operator from OUTSIDE the classified monoid family.
  *
  * The point of this operator is not that top-k is useful. It is that the socket carries an operator with a different
  * algebra, a different beat layout and no arithmetic in common with the monoid cell -- no key front end, no
  * exponential, no FMA -- while satisfying the same four obligations. So the tests below are deliberately the CONTRACT
  * tests, not a feature list:
  *
  * T-bottom-k keyPol = 1 keeps the k SMALLEST instead -- the same network, a KMV / MinHash sketch merge T-O2 closure
  * the output of one instance is a legal input to the next, checked against a 3-input reference T-O3 identity a short
  * list is padded with the identity and the live entries are unaffected T-O1 latency retire is fire delayed by the
  * published constant T-O4 stateless a pair's result does not depend on the pairs before it
  */
class TopKJunctionTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  private def topK(entries: Seq[(Double, Double)]): Seq[(Double, Double)] = entries.sortBy(-_._1).take(k)

  /** a random DESCENDING sorted list of `live` distinct values, padded to k with the identity.
    *
    * Values are ROUND-TRIPPED THROUGH FP32 first. The beat carries FP32, so a double-precision stimulus would make the
    * software reference disagree with the hardware in the last few bits of the mantissa for the right answer -- and
    * this operator only ever selects and reorders values, never computes new ones, so with an exactly-representable
    * stimulus the comparison can and should be exact.
    */
  private def randList(rng: Random, live: Int, tag: Int): Seq[(Double, Double)] = {
    val vs = Seq
      .fill(live)(rng.between(-50.0, 50.0))
  ...
  "TopK_merge" should "produce the k largest of the union, descending" in {
    test(new DataPathJunctionHarness(hasTopK)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (trial <- 0 until 8) {
        ...
        val out  =
          runPair(dut, csrWord(k), pack(A.map(_._1), A.map(_._2)), pack(B.map(_._1), B.map(_._2)), bDelay = trial % 3)
        val gold = topK(A ++ B)
    ...
    test(new DataPathJunctionHarness(hasTopK)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (trial <- 0 until 6) {
        ...
        val out  = runPair(
          dut,
          csrWord(k, keyPol = 1),
          pack(A.map(_._1), A.map(_._2)),
          pack(B.map(_._1), B.map(_._2)),
          bDelay = trial % 3
        )
    ...
    test(new DataPathJunctionHarness(hasTopK)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      val got = o4Cases.indices.reverse.map { i =>
        ...
        i -> runPair(
          dut,
          csrWord(k),
          pack(la.map(_._1), la.map(_._2)),
          pack(lb.map(_._1), lb.map(_._2)),
          bDelay = i % 4
        )
      }.toMap
