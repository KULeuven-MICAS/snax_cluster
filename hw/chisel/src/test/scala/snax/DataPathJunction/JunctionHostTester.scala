...
/** THE SOCKET TEST. Everything here is about the host, not about either operator's arithmetic.
  *
  * The socket's promise is that an operator is a PLUG-IN: you hand the host a list, each entry gets its own user CSR
  * words, and the enable bitmask arms exactly one per transfer. That promise has never been exercised with more than
  * one entry -- the deployed configuration has always been a single-element list -- so the CSR allocation in
  * `connectCfgWithList` has never actually had to distribute anything.
  *
  * The words below are chosen so that a MIS-ROUTED CSR IS DETECTABLE. The monoid word read as a linear one decodes to
  * an FP16 ADD instead of the BF16 MUL that was asked for; the linear word read as a monoid one decodes to a different
  * nValid and a different field count than the geometry that was asked for. Both are plainly different from the right
  * answer, which a lazier choice of words would not have been.
  */
class JunctionHostTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  private def operators: Seq[HasDataPathJunction] =
    Seq(
  ...
  "JunctionHost_twoOperators" should "give each operator its own CSR words and arm exactly one" in {
    ...
    test(new JunctionHostHarness(operators)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (i <- 0 until lanes) {
        ...
        assert(
          math.abs(got - gold) <= math.abs(gold) * 1e-2 + 1e-3,
      ...
      for (k <- 0 until pairSlots) {
        ...
        assert(
          math.abs(laneF32(outM, k) - gm)                       <= math.abs(gm) * 1e-6 + 1e-9,
  ...
  "JunctionHost_substituteMissingOperand" should "let the socket stand in for a shard that never arrives" in {
    ...
    def runOneSided(dut: JunctionHostHarness, enable: Int, csrs: Seq[BigInt], beat: BigInt, onA: Boolean): BigInt = {
      for ((c, i) <- csrs.zipWithIndex) dut.io.cfg.userCsr(i).poke(c.U)
      dut.io.cfg.enable.poke(enable.U)
      dut.io.start.poke(true); dut.clock.step(1); dut.io.start.poke(false)
    ...
    test(new JunctionHostHarness(operators)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      assert(
        runOneSided(dut, (1 << MON_SLOT) | (1 << 17), csrs, live, onA = true) == live,
        "substituting B did not reproduce the surviving operand"
      )
      // A's shard never arrives -- the same, from the other side
      assert(
        runOneSided(dut, (1 << MON_SLOT) | (1 << 16), csrs, live, onA = false) == live,
