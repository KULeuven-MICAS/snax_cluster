...
class DataPathJunctionHarness(junction: HasDataPathJunction) extends Module with RequireAsyncReset {
  ...
  io.identity_o   := dut.io.identity_o
...
object JunctionTestUtils {
  ...
  /** Drive one operand PAIR through the junction and return the folded beat. Both operands are offered concurrently on
    * independent threads, so the join is exercised with real (nonzero, varying) skew.
    */
