...
/** O5 -- the declared schema. Every operator on the socket reports, from its CSR word alone, that it cannot
  * honour the configuration it was handed. The point is not the check itself but WHAT IT REPLACES: without it
  * each word below produces a finite, format-legal, plausible beat that no downstream hop can tell from a
  * correct one, which is exactly the species of bug this block has shipped twice.
  *
  * The port reports; it does not refuse. Each case below also asserts the datapath still retires, because a fold
  * that stops mid-chain is worse than one that completes and is flagged.
  */
class JunctionCfgErrTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "CfgErr_monoid" should "flag a saturated sigma, an overflowing role count, and no live slot" in {
    test(new DataPathJunctionHarness(new HasMonoidJunction())).withAnnotations(
      Seq(VerilatorBackendAnnotation, flags)) { dut =>
  ...
  "CfgErr_topk" should "flag a live count its tuple cannot hold" in {
    test(new DataPathJunctionHarness(new HasTopKJunction(k = 8))).withAnnotations(
      Seq(VerilatorBackendAnnotation, flags)) { dut =>
  ...
  "CfgErr_bitwise" should "flag the encodable op that does not exist" in {
    test(new DataPathJunctionHarness(new HasBitwiseJunction())).withAnnotations(
      Seq(VerilatorBackendAnnotation, flags)) { dut =>
