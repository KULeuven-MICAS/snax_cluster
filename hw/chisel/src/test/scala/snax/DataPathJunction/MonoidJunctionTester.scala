...
/** Tier-1 for `MonoidJunction` -- the nonlinear 2->1 collective fold, and the socket's twisted-family operator.
  *
  * This is the operator's own test, and it carries what `JunctionSocketTester` cannot: the tree's only INDEPENDENT
  * double-precision goldens. The socket tester compares against FROZEN beats, which proves agreement with a captured
  * result rather than correctness of the arithmetic; here every expected value is recomputed from the algebra. It also
  * holds the only end-to-end C1 chain-closure proof.
  *
  * What these tests pin down, beyond "the arithmetic is right":
  *
  *   1. Every geometry folds a live operand PAIR against a numeric golden computed in double precision. 2. CHAINING:
  *      folding P = 4 partials as three successive pairwise junction ops reproduces the direct global answer, which is
  *      the reduce-along-the-route property at component level. 3. CUT-THROUGH: the first output appears after the SAME
  *      number of cycles whether the payload is 1 beat or 64. A store-and-forward join would make first-output latency
  *      scale with payload. 4. The fold streams at ~1 beat pair per cycle, since the combine holds no state between
  *      pairs. 5. Bypass discipline and the starvation watchdog -- the two properties the Junction CLASS guarantees.
  */
class MonoidJunctionTester extends AnyFlatSpec with ChiselScalatestTester {
    ...
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (trial <- 0 until 6) {
        ...
        for (k <- 0 until pairSlots) {
          ...
          assert(
            math.abs(laneF32(out, pairSlots + k) - gv) <= math.abs(gv) * 1e-5 + 1e-6,
    ...
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (trial <- 0 until 4) {
        ...
        for (k <- 0 until pairSlots) {
          ...
          assert(
            laneF32(out, pairSlots + k) == (if (aWins) k.toDouble else (pairSlots + k).toDouble),
  ...
  private def scanStep(a: (Double, Double), b: (Double, Double)): (Double, Double) = (a._1 * b._1, b._1 * a._2 + b._2)

  /** The state update is a SUM of two terms, so its error must be judged against the larger term, not against the
    * result: `k_B*v_A + v_B` can cancel to nearly zero, and relative error on a cancelling sum is unbounded in any
    * finite precision. This is the standard way to state a floating-point summation bound.
    */
  ...
  "MonoidJunction_scan" should "fold a chunked linear-attention recurrence slot-wise" in {
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      println(
        f"[Junction/scan] $pairSlots independent (decay, state) recurrences per beat pair, " +
  ...
  "MonoidJunction_scan_chain" should "reproduce the global recurrence when 4 chunks fold along a chain" in {
    ...
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (k <- 0 until pairSlots) {
        ...
        val scale = (1 until 4)
          .foldLeft((chunks(0)(k), 0.0)) { case ((st, mx), h) =>
  ...
  "MonoidJunction_scan_ordered" should "be deliberately NOT commutative, with A the earlier chunk" in {
    ...
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (k <- 0 until pairSlots) {
        ...
        assert(
          math.abs(laneF32(ab, pairSlots + k) - gv) / sumScale(pa(k), pb(k)) <= 1e-6,
  ...
  "MonoidJunction_scan_identity" should "pad a dead slot with (1, 0), the scan's identity" in {
    ...
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (k <- 0 until live) {
        ...
        assert(math.abs(laneF32(out, pairSlots + k) - gv) / sumScale(pa(k), pb(k)) <= 1e-6, s"live slot $k state")
      }
