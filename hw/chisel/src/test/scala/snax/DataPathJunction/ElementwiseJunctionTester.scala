...
/** Tier-1 for `ElementwiseJunction` -- the linear 2->1 fold.
  *
  * Covers the four ops at FP16, the runtime format select (BF16 and FP32 out of one netlist), the INTEGER grid and the
  * three properties that only it has -- exactness, route-independence and idempotent max -- the streaming and
  * cut-through behaviour, and the class-level bypass property.
  *
  * The last test is the reason the monoid class exists at all: it shows directly that a per-element reduction applied
  * to online-softmax statistics computes the WRONG answer, because the softmax merge needs a rescale derived from the
  * operand pair and is not a per-element operation at any granularity.
  */
class ElementwiseJunctionTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  private def unpack(beat: BigInt, f: Fmt): Seq[Double] = (0 until f.lanes).map(i => f.dec(lane(beat, i, f.width)))

  private def hasEw = new HasElementwiseJunction(elemWidth = 16, fpPipe = 1)

  private def checkOp(
    dut:    DataPathJunctionHarness,
    f:      Fmt,
    op:     Int,
    opName: String,
  ...
  "ElementwiseJunction_routeIndependence" should "return ONE answer over every reduction order, in integer" in {
    ...
    test(new DataPathJunctionHarness(hasEw)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      val fp32 = Fmt(
        FMT_FP32,
        32,
        d => BigInt(java.lang.Float.floatToIntBits(d.toFloat).toLong & 0xffffffffL),
        b => java.lang.Float.intBitsToFloat(b.toInt).toDouble,
        "FP32"
      )
      val rf   = foldAll(csrWord(OP_ADD, FMT_FP32), i => pack(sf(i), fp32)).distinct

      println(
        f"[EW/route] ${orders.length} reduction orders of 4 shards: " +
  ...
  "ElementwiseJunction_vs_monoid" should "be provably unable to express the softmax merge" in {
    ...
    test(new DataPathJunctionHarness(hasEw)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      println(
        f"[EW vs Monoid] linear 2-input reduction on (m,l): got (${out(0)}%.4f, ${out(1)}%.4f), " +
