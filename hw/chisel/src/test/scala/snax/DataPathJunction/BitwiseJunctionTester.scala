...
/** Tier-1 for `BitwiseJunction` -- the operator with no arithmetic.
  *
  * The tests are the CONTRACT tests, because the arithmetic is one gate and needs no defending. What is being
  * checked is that an operator containing no adder, no multiplier, no comparator and no lane structure still
  * satisfies the same four obligations the twisted family does.
  */
