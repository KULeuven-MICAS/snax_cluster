...
/** Tier-1 for the JUNCTION SOCKET -- the 2->1 operator contract, exercised on the two shipped operators.
  *
  * The headline check is an EQUIVALENCE, not a golden: the merged netlist must reproduce, BIT FOR BIT, what the two
  * predecessor junctions produce for the same CSR word and the same operand pair -- `ElementwiseJunction` for the
  * linear per-element class, and `MonoidJunction` for the structured serving-reduction class, with both classes
  * exercised on ONE elaborated instance. That is what makes the merge a pure area/reuse move rather than a redesign,
  * and it is the artifact behind the "N operators, one datapath" claim.
  *
  * Because chiseltest gives each scalatest CASE one Verilator build directory, the two reference legs are separate
  * cases that hand their beats to the equivalence case through the vars below.
  */
class JunctionSocketTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  /** Each case draws its operands from its OWN seeded generator, keyed by the case name. A shared stream would couple
    * the cases to each other: adding or removing one would re-draw every case after it and move every frozen beat
    * below, which would make the table impossible to maintain and impossible to trust.
    */
  private val monoidCases: Seq[Vec3] = {
    ...
    Seq(
      ...
      Vec3(
        "ARGMAX",
        csrMonoid(n = 1, nExp = 0, nAdd = 0, sigma = 3, nValid = pairSlots),
        packPairs(idx(rArg, 0)),
        packPairs(idx(rArg, pairSlots))
      ),
      Vec3(
        "ATTN",
        csrMonoid(n = 1 + dHead, nExp = 1 + dHead, nAdd = 0, sigma = 0, nValid = 1),
        packSingle(rAttn.between(-2.0, 5.0), rAttn.between(1.0, 5.0), Seq.fill(dHead)(rAttn.between(-3.0, 3.0))),
        packSingle(rAttn.between(-2.0, 5.0), rAttn.between(1.0, 5.0), Seq.fill(dHead)(rAttn.between(-3.0, 3.0)))
      ),
      Vec3(
        "MOMENT2",
        csrMonoid(n = 3, nExp = 3, nAdd = 0, sigma = 0, nValid = 1),
        packSingle(rM2.between(-2.0, 5.0), rM2.between(1.0, 5.0), Seq(rM2.between(-4.0, 4.0), rM2.between(0.5, 6.0))),
        packSingle(rM2.between(-2.0, 5.0), rM2.between(1.0, 5.0), Seq(rM2.between(-4.0, 4.0), rM2.between(0.5, 6.0)))
      )
    )
  }
  ...
  private val GOLDEN_LINEAR: Map[String, BigInt] = Map(
    "FP16/ADD" -> g(
      "34164434c59c40cc3c06c58a46dc34053c77420a4000414ec4f242a73c8cbd0c3cf5baecb530b10040fd403cb070410244d4430db954c32e4648bee1bd26c19a"
    ),
    "FP16/MUL" -> g(
      "4482caf3bd9d416645acbde045edc4bb2430400a3e0c2fac4814c85d3d01c5f92f67c781362e428f42f1c3d43fc43c1c439e42c94390415ec8de4225afd5c063"
    ),
    "FP16/MAX" -> g(
      "be5a41fb4073418840413d6e436e3ba6c0164084427e40a2c1273d39c200433842752df8b17240c13f3640c2433a436541c340babcbfb5d63cbd40863e654065"
    ),
    "FP16/MIN" -> g(
      "bcf9c376c019be453e8ec3b4c31cc3ccc2d2c24fbead3e16be0ebe7d22b5c21a4174c001a82f1ea73ffbc36fc13f3c55c2d3c16432883d56be81bfecc2232c9f"
    ),
    "BF16/ADD" -> g(
      "bf9cc0c8bf12c05c40b1c035c0103fb8c0183f6c408e4054c01e3ed8bfacbf18c046c01140e5c0ac3ff53da0bef83f3cc07c3f0c40c7c0864064bfd23facbee2"
    ),
    "FP32/MUL" -> g(
      "41202bcb3eb8d0bfbff4d2a5c0446521c076510fc0f6cd9f3ffc5988bdbf8d4ec1075146c0ef636ec0c0715440b7da5d40ba79ddc072eae83f591936bfc3eb60"
    )
  )

  private val GOLDEN_MONOID: Map[String, BigInt] = Map(
    "MOMENT"  -> g(
      "401c27a94094b18e4017585a3ff746f14032cf0c4032e873408e4f504039d6f440b31f68409f570d40b62dd040b1780f408329424078c80d40adc9ec408f7756"
    ),
    ...
    "ARGMAX"  -> g(
      "40e0000041600000415000004140000040400000412000003f8000000000000040a2da95405bc4c94100939540d9bcb2c0a60dd9c0bde6144016b8eb410e886b"
    ),
  ...
  "Socket_geometryReach" should "run operators no fixed elaboration parameter could express" in {
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      val d6 = 6
      val sh = (0 until 2).map(_ =>
        (0 until 2).map(_ => (rng.between(-2.0, 5.0), rng.between(1.0, 5.0), Seq.fill(d6)(rng.between(-3.0, 3.0))))
      )
      ...
      val outA = runPair(
        dut,
        csrMonoid(n = 1 + d6, nExp = 1 + d6, nAdd = 0, sigma = 1, nValid = 2),
        packS2(sh(0)),
        packS2(sh(1))
      )
      for (s <- 0 until 2) {
      ...
      val oMin = runPair(
        dut,
        csrMonoid(n = 1, nExp = 0, nAdd = 0, sigma = 3, nValid = nv, keyPol = 1),
        packPairs(pa),
        packPairs(pb)
      )
      for (k <- 0 until nv) {
        ...
        assert(
          laneF32(oMin, pairSlots + k) == (if (la(k) <= lb(k)) k.toDouble else (pairSlots + k).toDouble),
      ...
      println(
        "[Socket/beyondEnum] ATTN dHead=6 at 2 partials/beat, ARGMIN on a short beat, and flash+argmax " +
  ...
  "Socket_sigmaSaturation" should "degrade an illegal sigma instead of interleaving partials" in {
    ...
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      val silly =
        csrMonoid(n = 1 + dHead, nExp = 1 + dHead, nAdd = 0, sigma = 3, nValid = 1) // F = 10 with sigma = 3: impossible
  ...
  "Socket_moment2Dense" should "fold FOUR moment banks per beat at sigma=2" in {
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      println(
        "[Socket/MOMENT2-dense] 4 moment banks per beat (was 1), 16/16 lanes live, vs a double-precision " +
    ...
    test(new DataPathJunctionHarness(hasMonoid)).withAnnotations(Seq(VerilatorBackendAnnotation, flags)) { dut =>
      ...
      for (trial <- 0 until 2) {
        ...
        for (hop <- 1 until 4)
          acc = runPair(
            dut,
            csrMonoid(n = 1, nExp = 1, nAdd = 0, sigma = 3, nValid = pairSlots),
            acc,
            packPairs(shards(hop)),
            bDelay = hop
          )
      ...
      for (trial <- 0 until 2) {
        ...
        for (hop <- 1 until 4)
          acc = runPair(
            dut,
            csrMonoid(n = 3, nExp = 3, nAdd = 0, sigma = 2, nValid = 4),
            acc,
            packDense4(shards(hop)),
            bDelay = hop
          )
        for (s <- 0 until 4) {
          ...
          assert(
            math.abs(laneF32(acc, 8 + s) - ga) / (math.abs(ga) + 1e-6) <= 5e-2,
            s"chain sigma=2 trial $trial slot $s A"
          )
      ...
      for (hop <- 1 until 4)
        acc = runPair(
          ...
          csrMonoid(n = 1 + dHead, nExp = 1 + dHead, nAdd = 0, sigma = 0, nValid = 1),
          acc,
          packSingle(ash(hop)._1, ash(hop)._2, ash(hop)._3),
          bDelay = hop
        )
      val gm = ash.map(_._1).max
      assert(math.abs(laneF32(acc, 0) - gm)                      <= math.abs(gm) * 1e-6 + 1e-9, "chain sigma=0 m")
      assert(
        math.abs(laneF32(acc, 1) - ash.map { case (m, l, _) => l * math.exp(m - gm) }.sum) /
          ash.map { case (m, l, _) => l * math.exp(m - gm) }.sum <= 4e-2,
        "chain sigma=0 l"
      )
      for (j <- 0 until dHead) {
      ...
      println(
        "[Socket/closure] P=4 hop-by-hop == the direct global merge at sigma = 3, 2 and 0 -- C1 holds " +
