...
class AddressGenUnitTester extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "AddressGenUnit: 16, 16" should " pass" in test(new AddressGenUnit(AddressGenUnitParam()))
  ...
  "AddressGenUnit: 8, 16" should " pass" in test(new AddressGenUnit(AddressGenUnitParam()))
    .withAnnotations(Seq(WriteVcdAnnotation, VerilatorBackendAnnotation)) { dut =>
      dut.io.cfg.Ptr.poke(100000.U)
